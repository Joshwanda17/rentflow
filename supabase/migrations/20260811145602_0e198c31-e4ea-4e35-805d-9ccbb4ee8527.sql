DROP FUNCTION IF EXISTS public.fin_ops_recent_cash_codes(integer);

CREATE OR REPLACE FUNCTION public.fin_ops_recent_cash_codes(p_limit integer DEFAULT 50)
 RETURNS TABLE(verification_id uuid, deposit_request_id uuid, depositor_name text, wallet_holder_name text, cash_owner_name text, depositor_phone text, amount numeric, code text, status text, attempts integer, max_attempts integer, deposit_purpose text, cash_location text, expires_at timestamp with time zone, created_at timestamp with time zone)
 LANGUAGE plpgsql
