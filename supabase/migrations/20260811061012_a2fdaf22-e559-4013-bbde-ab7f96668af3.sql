  ...
  SELECT jsonb_build_object(
    ...
    'paid_today', coalesce((
      SELECT jsonb_agg(x ORDER BY (x->>'roi')::numeric DESC) FROM (
        SELECT jsonb_build_object(
          'name', coalesce(owner.full_name, ip.account_name, holder.full_name, 'Partner'),
          'phone', coalesce(owner.phone, holder.phone),
          'roi', gl.amount,
          'portfolio_code', ip.portfolio_code,
          'principal', coalesce(ip.investment_amount, 0),
          'paid_to', CASE WHEN owner.id IS NOT NULL AND owner.id <> gl.user_id
        ...
        LEFT JOIN profiles owner ON owner.id = ip.investor_id
        WHERE gl.category = 'roi_wallet_credit' AND gl.created_at >= d0 AND gl.created_at < d1
    ...
    'promissory_notes', (
      SELECT jsonb_build_object(
        ...
        'verified_count', (SELECT count(*) FROM promissory_notes WHERE status IN ('approved','activated','verified')),
        'verified_amount', (SELECT coalesce(sum(amount),0) FROM promissory_notes WHERE status IN ('approved','activated','verified')),
        'rows', coalesce((
          SELECT jsonb_agg(jsonb_build_object(
            'partner_name', coalesce(pn.partner_name, pu.full_name, 'Partner'),
            'agent_name', coalesce(ag.full_name, 'Agent'),
            'amount', coalesce(pn.amount,0),
            'status', pn.status,
            'at', pn.created_at
          ) ORDER BY pn.amount DESC)
          FROM promissory_notes pn
          LEFT JOIN profiles ag ON ag.id = pn.agent_id
          LEFT JOIN profiles pu ON pu.id = pn.partner_user_id
          WHERE pn.status = 'pending'
             OR (pn.created_at >= d0 AND pn.created_at < d1)
             OR (pn.status IN ('approved','activated','verified') AND pn.updated_at >= d0 AND pn.updated_at < d1)
        ), '[]'::jsonb)
      )
    ),
    'withdrawals', (
      SELECT jsonb_build_object(
        ...
        'partner_linked_count', (
          SELECT count(*) FROM withdrawal_requests
          WHERE linked_party IS NOT NULL
            AND status IN ('completed','fin_ops_approved','approved','cfo_approved','coo_approved','manager_approved','processing')
        ),
        'partner_linked_amount', (
        ...
        'rows', coalesce((
          SELECT jsonb_agg(jsonb_build_object(
            'name', coalesce(partner.full_name, w.mobile_money_name, w.bank_account_name, holder.full_name, 'Partner'),
            'agent_name', holder.full_name,
            'method', coalesce(w.fin_ops_payment_method, w.payout_method, 'mobile money'),
          ...
          LEFT JOIN profiles partner ON partner.id = w.linked_party
          LEFT JOIN profiles holder ON holder.id = w.user_id
        ), '[]'::jsonb)
      )
    )
  ) INTO result;
