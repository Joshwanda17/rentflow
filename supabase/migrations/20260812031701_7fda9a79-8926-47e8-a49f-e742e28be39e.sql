CREATE OR REPLACE FUNCTION public.ops_landlord_funded_stats(p_date_from timestamp with time zone DEFAULT NULL::timestamp with time zone, p_date_to timestamp with time zone DEFAULT NULL::timestamp with time zone, p_search text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
...
BEGIN
  ...
  first_ever AS (
    SELECT rr.landlord_id AS fe_landlord_id, min(rr.funded_at) AS first_funded_at
    FROM public.rent_requests rr
    WHERE rr.funded_at IS NOT NULL AND rr.landlord_id IS NOT NULL
  ...
  SELECT jsonb_build_object(
    ...
    'summary', (
      SELECT jsonb_build_object(
        'landlords_funded', count(DISTINCT c.landlord_id),
        'requests_funded',  count(*),
        'total_funded',     coalesce(sum(c.rent_amount), 0),
        'total_repayment',  coalesce(sum(c.total_repayment), 0),
        'total_fees',       coalesce(sum(coalesce(c.access_fee,0) + coalesce(c.request_fee,0)), 0),
        'avg_per_landlord', CASE WHEN count(DISTINCT c.landlord_id) = 0 THEN 0
                                 ELSE round(coalesce(sum(c.rent_amount),0) / count(DISTINCT c.landlord_id), 0) END,
        'avg_per_request',  CASE WHEN count(*) = 0 THEN 0
                                 ELSE round(coalesce(sum(c.rent_amount),0) / count(*), 0) END,
        'verified_landlords', count(DISTINCT c.landlord_id) FILTER (WHERE c.landlord_verified),
        'unverified_landlords', count(DISTINCT c.landlord_id) FILTER (WHERE NOT coalesce(c.landlord_verified,false)),
        'with_momo', count(DISTINCT c.landlord_id) FILTER (WHERE nullif(btrim(coalesce(c.mobile_money_number,'')),'') IS NOT NULL),
        'with_bank', count(DISTINCT c.landlord_id) FILTER (WHERE nullif(btrim(coalesce(c.bank_name,'')),'') IS NOT NULL),
        'districts_covered', count(DISTINCT c.district),
        'agents_involved', count(DISTINCT c.agent_name) FILTER (WHERE c.agent_name <> 'Unassigned'),
      ...
      FROM cur c LEFT JOIN first_ever fe ON fe.fe_landlord_id = c.landlord_id
    ),
    'previous', (
      SELECT jsonb_build_object(
        'landlords_funded', count(DISTINCT p.landlord_id),
        'requests_funded',  count(*),
        'total_funded',     coalesce(sum(p.rent_amount), 0),
        'total_repayment',  coalesce(sum(p.total_repayment), 0),
        'districts_covered', count(DISTINCT p.district)
      ) FROM prev p
    ),
    'by_district', coalesce((
      ...
      FROM (
        ...
        FROM cur c GROUP BY c.district
      ) x
    ), '[]'::jsonb),
    'by_agent', coalesce((
      ...
      FROM (
        SELECT jsonb_build_object(
          ...
          'service_centre', min(c.service_centre),
          'landlords_funded', count(DISTINCT c.landlord_id),
          'requests_funded', count(*),
          'total_funded', coalesce(sum(c.rent_amount),0),
          'districts', count(DISTINCT c.district),
          'previous_landlords_funded', (
        ...
        FROM cur c GROUP BY c.agent_name
      ) x
    ), '[]'::jsonb),
    'by_service_centre', coalesce((
      ...
      FROM (
        SELECT jsonb_build_object(
          ...
          'agents', count(DISTINCT c.agent_name),
          'landlords_funded', count(DISTINCT c.landlord_id),
          'requests_funded', count(*),
          'total_funded', coalesce(sum(c.rent_amount),0),
          'previous_landlords_funded', (
        ...
        FROM cur c GROUP BY c.service_centre
      ) x
    ), '[]'::jsonb),
    'daily', coalesce((
      ...
      FROM (
        SELECT jsonb_build_object(
          'day', to_char(date_trunc('day', c.funded_at), 'YYYY-MM-DD'),
          'landlords_funded', count(DISTINCT c.landlord_id),
          'requests_funded', count(*),
          'total_funded', coalesce(sum(c.rent_amount),0)
        ) AS t
        FROM cur c GROUP BY date_trunc('day', c.funded_at)
      ) x
    ), '[]'::jsonb),
    'rows', coalesce((
