...
BEGIN
  ...
  INSERT INTO audit_logs (action_type, table_name, record_id, user_id, metadata)
  VALUES ('payout_proof_attached', 'withdrawal_requests', p_withdrawal_id::text, auth.uid(),
          jsonb_build_object(
            'reason', 'Merchant attached missing payout proof after settlement',
            'path', p_path,
            'bucket', COALESCE(p_bucket, 'payment-proofs'),
            'proof_type', p_type
          ));
