...
DECLARE
  ...
  v_top jsonb;
BEGIN
  -- EAT (UTC+3) calendar day boundaries for the requested date.
  v_start := (p_date::timestamp AT TIME ZONE 'UTC') - interval '3 hours';
  v_end := v_start + interval '1 day';
  v_prev_start := v_start - interval '1 day';

  -- Daily activity counts (all created within the EAT day).
  ...
  -- Active agents yesterday: distinct agents who performed any tracked field action.
  WITH active AS (
  ...
  -- Active sub-agents yesterday: sub-agents that were among the active agents.
  ...
  -- Top active agents by combined activity yesterday.
  WITH acts AS (
  ...
  FROM (
    SELECT
      agg.agent_id,
      coalesce(p.name, p.full_name, 'Unknown') AS name,
      p.phone,
