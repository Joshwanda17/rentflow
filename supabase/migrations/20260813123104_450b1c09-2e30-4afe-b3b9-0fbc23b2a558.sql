...
BEGIN
  ...
  money AS (
    SELECT
      ...
      (SELECT COALESCE(sum(daily_repayment), 0) FROM v_tenant_daily_eligibility) AS expected_daily,
  ...
  -- rents that are still being collected on: same population as the
  -- "Expected daily" KPI (v_tenant_daily_eligibility base)
  live_rents AS (
    ...
    WHERE status IN ('funded', 'repaying')
      ...
      AND COALESCE(agent_payment_status, 'paying') <> 'not_paying'
  ),
  -- every rent that has ever been funded: used for the cumulative
  -- outstanding balance so historic collections stay matched to a plan
  booked_rents AS (
