...
BEGIN
  ...
  -- Wallet identity row lives in wallets_physical; public.wallets is a view.
  INSERT INTO public.wallets_physical (user_id)
  VALUES (p_agent_id)
  ON CONFLICT (user_id) DO NOTHING;

  -- Read current cached buckets from the wallets view (no FOR UPDATE — views can't be locked).
  -- Concurrency is enforced by the ledger triggers + apply_wallet_movement sole-writer rule.
  SELECT
