...
BEGIN
  ...
  -- Identity row (wallets is a view; the physical table is wallets_physical)
  INSERT INTO public.wallets_physical (user_id)
  VALUES (p_agent_id)
  ON CONFLICT (user_id) DO NOTHING;

  -- Buckets are derived from the ledger via the wallets view. No FOR UPDATE on a view.
  SELECT
  ...
  -- Single sanctioned ledger entry point. Posts 4 balanced legs, stamps a
  -- shared transaction_group_id, and lets AFTER triggers update wallet caches
  -- (pivot + buckets) automatically. We pass skip_balance_check because the
  -- check inside create_ledger_transaction is a withdrawable-bucket check,
  -- whereas this flow draws from the float bucket (already validated above).
  v_txn_group := public.create_ledger_transaction(
    jsonb_build_array(
      -- Agent float-out (operational_wallet → float bucket)
      jsonb_build_object(
        'user_id',       p_agent_id,
        'amount',        p_amount,
        'direction',     'cash_out',
      ...
      -- Landlord cash-in (user → withdrawable bucket)
      jsonb_build_object(
      ...
      -- Agent commission (10%) credit (user → withdrawable bucket)
      jsonb_build_object(
        'user_id',       p_agent_id,
      ...
      -- Platform commission expense (matches wallet credit on the platform book)
      jsonb_build_object(
        'user_id',       p_agent_id,
        'amount',        v_commission_earned,
        'direction',     'cash_out',
    ...
    true     -- skip_balance_check (float bucket already validated above)
  );
