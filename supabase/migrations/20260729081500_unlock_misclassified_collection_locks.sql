-- Reverse the erroneous 2026-07-29 collection locks applied by
-- notify-agent-collection-lapse when it defaulted every tenant without a
-- subscription_charges row to "daily" (lock at 8 idle days), sweeping in
-- weekly-cadence tenants who were still well within their 15-day grace window.
--
-- Unlock rule for locks stamped in the last 24 hours:
--   * fewer than 2 collection rows ever          → unknown cadence, unlock
--   * median gap between last 5 collections ≥ 4d → weekly, unlock unless
--                                                   idle for ≥ 15 days
...
to_unlock AS (
  ...
  WHERE
    COALESCE(s.gap_count, 0) < 1                  -- < 2 collections → unknown
    OR (
      s.median_gap >= 4                            -- weekly cadence
