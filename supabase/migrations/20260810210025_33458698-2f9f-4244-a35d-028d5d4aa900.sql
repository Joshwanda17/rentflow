...
BEGIN
  ...
  -- Partner Ops portfolio source of truth (same statuses the Partner Ops dashboard reads)
  CREATE TEMP TABLE IF NOT EXISTS _cf_pf (
    id uuid, investor_id uuid, investment_amount numeric, roi_percentage numeric,
    anchor_date date, maturity_date date
  ) ON COMMIT DROP;
  TRUNCATE _cf_pf;
  ...
  SELECT ip.id,
         ...
         COALESCE(ip.roi_percentage, 15),
         CASE
           WHEN ip.next_roi_date IS NOT NULL THEN ip.next_roi_date::date
           ELSE (
             -- first payout = created_at + 1 month on payout_day, rolled forward to today
             SELECT d FROM (
               SELECT (date_trunc('month', ip.created_at AT TIME ZONE v_tz)::date
                        + ((n + 1) || ' month')::interval)::date
                      + (LEAST(COALESCE(ip.payout_day, EXTRACT(DAY FROM (ip.created_at AT TIME ZONE v_tz))::int), 28) - 1)
                      AS d
  ...
  WITH occ AS (
    SELECT pf.id, pf.investor_id,
           ...
           pf.maturity_date
    FROM _cf_pf pf CROSS JOIN generate_series(0, 36) AS n
  ...
  WITH occ AS (
    SELECT pf.id, pf.investor_id, pf.investment_amount,
           (pf.anchor_date + (n || ' month')::interval)::date AS due_date,
           ROUND(pf.investment_amount * pf.roi_percentage / 100.0) AS amount,
           pf.maturity_date
    FROM _cf_pf pf CROSS JOIN generate_series(0, 36) AS n
    ...
    SELECT w.investor_id,
           ...
           (SELECT SUM(pf2.investment_amount) FROM _cf_pf pf2 WHERE pf2.investor_id = w.investor_id) AS committed
  ...
  RETURN jsonb_build_object(
    ...
    'categories', v_cats,
    'partners', v_partners,
    'portfolio_count', (SELECT COUNT(*) FROM _cf_pf),
    'committed_capital', COALESCE((SELECT SUM(investment_amount) FROM _cf_pf), 0)
  );
END;
$function$;