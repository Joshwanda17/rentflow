...
RETURNS TABLE(
  deposit_request_id uuid, gmail_transaction_id uuid, method text, amount numeric,
  matched_transaction_id text, user_id uuid, provider text, counterparty text,
  internal_date timestamptz, signals text[], match_score integer,
  from_email text, from_name text, subject text, snippet text,
  depositor_email text, depositor_full_name text, depositor_phone text
)
LANGUAGE plpgsql
SECURITY DEFINER
