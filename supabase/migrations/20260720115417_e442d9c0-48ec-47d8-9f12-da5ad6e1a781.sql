...
DECLARE
  ...
  -- Portfolio-first vouch components
  v_angel_shares_ugx numeric := 0;
  v_vouch_portfolio numeric := 0;
  v_vouch_shares numeric := 0;
  v_vouch_wallet numeric := 0;
  v_vouch_network numeric := 0;
  v_vouch_agent numeric := 0;
  v_vouch_verification numeric := 0;
  v_vouch_behavior numeric := 0;
  v_vouch_payment numeric := 0;
  v_vouch_boosters numeric := 0;
...
BEGIN
  ...
  SELECT COALESCE(SUM(amount), 0) INTO v_wallet_outflow
    FROM wallet_transactions WHERE sender_id = v_user_id AND created_at > now() - interval '180 days';
  ...
  BEGIN
    SELECT COUNT(*) INTO v_subagents_all FROM agent_subagents WHERE parent_agent_id = v_user_id;
  ...
  BEGIN
    SELECT COUNT(*) INTO v_partners_managed FROM investor_portfolios
     WHERE agent_id = v_user_id AND status IN ('active','matured','pending','pending_approval');
  EXCEPTION WHEN OTHERS THEN v_partners_managed := 0; END;

  BEGIN
    SELECT COUNT(*) INTO v_landlords_registered FROM landlords WHERE registered_by = v_user_id;
  EXCEPTION WHEN OTHERS THEN v_landlords_registered := 0; END;

  BEGIN
    SELECT COUNT(*) INTO v_promissory_count FROM promissory_notes WHERE agent_id = v_user_id;
  ...
  IF v_always_gps THEN
    ...
    v_data_points := v_data_points + 1;
  END IF;
  IF v_location_count_30d >= 30 THEN v_verification_score := v_verification_score + 2;
  ELSIF v_location_count_30d >= 10 THEN v_verification_score := v_verification_score + 1;
  END IF;
  ...
  v_landlord_score := LEAST(5, (v_verified_listings * 1.0) + (v_landlord_listings * 0.3));
  IF v_landlord_listings > 0 THEN v_data_points := v_data_points + 1; END IF;
  ...
  BEGIN
    SELECT COALESCE(SUM(roi_amount), 0) INTO v_roi_paid_180d
      FROM supporter_roi_payments
     WHERE supporter_id = v_user_id AND status = 'paid'
       AND paid_at > now() - interval '180 days';
    SELECT COALESCE(SUM(roi_amount), 0) INTO v_roi_paid_30d
      FROM supporter_roi_payments
     WHERE supporter_id = v_user_id AND status = 'paid'
       AND paid_at > now() - interval '30 days';
  EXCEPTION WHEN OTHERS THEN v_roi_paid_180d := 0; v_roi_paid_30d := 0; END;
  ...
  -- ============ PORTFOLIO-FIRST WELILE VOUCH ============
  -- Primary vouch = full partnership portfolio value (1x).
  -- Welile shares (angel pool) add 2x their value.
  -- Wallet activity, network, agent performance, verification and
  -- movement behavior each add smaller boosters on top.
  BEGIN
    SELECT COALESCE(SUM(amount), 0)
      INTO v_angel_shares_ugx
      FROM angel_pool_investments
     WHERE investor_id = v_user_id
       AND status IN ('active','completed','confirmed','approved');
  EXCEPTION WHEN OTHERS THEN v_angel_shares_ugx := 0; END;

  v_vouch_portfolio    := GREATEST(0, COALESCE(v_portfolio_value, 0));
  v_vouch_shares       := GREATEST(0, COALESCE(v_angel_shares_ugx, 0)) * 2;

  v_vouch_wallet       := LEAST(200000, COALESCE(v_wallet_score, 0) * 20000);
  v_vouch_network      := LEAST(150000, COALESCE(v_network_score, 0) * 10000);
  v_vouch_agent        := LEAST(500000, COALESCE(v_agent_perf_score, 0) * 25000)
                        + GREATEST(0, COALESCE(v_agent_term, 0));
  v_vouch_verification := LEAST(100000, COALESCE(v_verification_score, 0) * 10000);
  v_vouch_behavior     := LEAST(75000,  COALESCE(v_behavior_score, 0) * 15000);
  v_vouch_payment      := GREATEST(0, COALESCE(v_total_paid, 0)) * 0.3
                        + GREATEST(0, COALESCE(v_combined_monthly_cashflow, 0)) * 0.25;

  v_vouch_boosters := v_vouch_wallet + v_vouch_network + v_vouch_agent
                    + v_vouch_verification + v_vouch_behavior + v_vouch_payment;

  v_borrowing_limit := ROUND(v_vouch_portfolio + v_vouch_shares + v_vouch_boosters, 0);
  ...
  RETURN jsonb_build_object(
    ...
    'trust', jsonb_build_object(
      ...
      'vouch_breakdown', jsonb_build_object(
        'total_ugx', v_borrowing_limit,
        'portfolio_component_ugx', ROUND(v_vouch_portfolio, 0),
        'angel_shares_ugx', ROUND(v_angel_shares_ugx, 0),
        'shares_component_ugx', ROUND(v_vouch_shares, 0),
        'boosters_ugx', ROUND(v_vouch_boosters, 0),
        'booster_breakdown', jsonb_build_object(
          'wallet_activity', ROUND(v_vouch_wallet, 0),
          'network_contribution', ROUND(v_vouch_network, 0),
          'agent_performance', ROUND(v_vouch_agent, 0),
          'verification', ROUND(v_vouch_verification, 0),
          'movement_behavior', ROUND(v_vouch_behavior, 0),
          'payment_history', ROUND(v_vouch_payment, 0)
        )
      ),
    ...
    'supporter_activity', jsonb_build_object(
      ...
      'angel_shares_ugx', v_angel_shares_ugx
    ),
