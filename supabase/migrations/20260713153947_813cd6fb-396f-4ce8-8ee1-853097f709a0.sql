...
BEGIN
  IF public.has_role(p_user_id, 'agent') THEN
    -- Sub-agent contribution (revised):
    --   +50,000 per registered (accepted) sub-agent
    --   +50,000 more once that sub-agent becomes ACTIVE
    --     (has a funded/repaying tenant they collect on)
    --   => registered sub = 50k, active sub = 100k total.
