...
-- Add the missing rent-via-float allocation categories to the locked allowlist.
-- These are legitimate, balanced ledger legs posted by agent_allocate_tenant_payment;
-- omitting them caused strict_mode to abort every agent float→tenant allocation
-- with "Category 'rent_payment_for_tenant' is not in the locked allowlist".
