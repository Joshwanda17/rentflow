...
BEGIN
  ...
  IF v_is_momo THEN
    -- Number: compare digits only, match on the national 9-digit suffix so
    -- 0780…, 256780…, +256 780… are treated as equal.
    ...
    -- Registered name: case-insensitive, punctuation-stripped, whitespace-
    -- collapsed exact match. Only enforced when a name is stored.
...
REVOKE ALL ON FUNCTION public.claim_withdrawal_verified(uuid, text, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.claim_withdrawal_verified(uuid, text, text) TO authenticated, service_role;