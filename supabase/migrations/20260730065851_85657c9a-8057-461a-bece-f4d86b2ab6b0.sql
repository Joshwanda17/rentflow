...
CREATE OR REPLACE FUNCTION public.cto_classify_error(p_msg text)
RETURNS jsonb
LANGUAGE sql
IMMUTABLE
...
AS $fn$
  SELECT CASE
    WHEN p_msg ILIKE '%indexed database%' OR p_msg ILIKE '%indexeddb%' OR p_msg ILIKE '%idb%' THEN
      jsonb_build_object('category','Offline storage','root_cause','IndexedDB connection reset while the offline cache was syncing (browser evicted or closed the database handle).','fix','Reinitialise the IndexedDB connection with exponential-backoff retry, reopen on the "close"/"versionchange" events, and replay queued offline transactions.','team','Frontend Platform')
    WHEN p_msg ILIKE '%script error%' THEN
      jsonb_build_object('category','Cross-origin script','root_cause','A cross-origin script threw an error; the browser masks the detail because the asset is served without CORS headers.','fix','Serve JS bundles with crossorigin="anonymous" + Access-Control-Allow-Origin and upload source maps so the real stack is captured.','team','Frontend Platform')
    WHEN p_msg ILIKE '%dynamically imported module%' OR p_msg ILIKE '%chunk%' OR p_msg ILIKE '%mime type%' THEN
      jsonb_build_object('category','Stale build','root_cause','The client is holding a cached HTML shell that points at code chunks deleted by a newer deploy.','fix','Keep HTML no-cache at the edge and force a single window.location.reload() when a chunk import fails.','team','Frontend Platform')
    WHEN p_msg ILIKE '%failed to fetch%' OR p_msg ILIKE '%networkerror%' OR p_msg ILIKE '%network request failed%' OR p_msg ILIKE '%load failed%' THEN
      jsonb_build_object('category','Network','root_cause','Request never reached the backend - mobile connectivity dropped or the edge function was unreachable.','fix','Wrap the call in retry-with-backoff, surface an offline state, and queue the mutation for replay instead of failing hard.','team','Frontend Platform')
    WHEN p_msg ILIKE '%non-2xx%' OR p_msg ILIKE '%edge function%' THEN
      jsonb_build_object('category','Edge function','root_cause','An edge function returned a non-2xx status; the browser SDK masks the body.','fix','Read error.context.text() on the client, log the upstream status/body, and fix the failing branch in the function.','team','Backend')
    WHEN p_msg ILIKE '%row-level security%' OR p_msg ILIKE '%permission denied%' OR p_msg ILIKE '%not authoris%' OR p_msg ILIKE '%not authoriz%' OR p_msg ILIKE '%403%' THEN
      jsonb_build_object('category','Authorisation','root_cause','A query was rejected by row-level security or a missing table grant for the caller role.','fix','Review the RLS policy and GRANTs for the table involved; scope the client query to the caller instead of widening the policy.','team','Backend Security')
    WHEN p_msg ILIKE '%jwt%' OR p_msg ILIKE '%401%' OR p_msg ILIKE '%invalid claim%' OR p_msg ILIKE '%session%expired%' THEN
      jsonb_build_object('category','Authentication','root_cause','The access token expired or was malformed before the request was made.','fix','Refresh the session before privileged calls and route users to re-authentication instead of retrying with a dead token.','team','Auth')
    WHEN p_msg ILIKE '%timeout%' OR p_msg ILIKE '%statement canceled%' OR p_msg ILIKE '%57014%' THEN
      jsonb_build_object('category','Latency','root_cause','The operation exceeded its time budget - usually an unindexed query or an oversized result set.','fix','Profile the query with EXPLAIN ANALYZE, add the covering index, and paginate the payload.','team','Backend')
    WHEN p_msg ILIKE '%quota%' OR p_msg ILIKE '%storage full%' THEN
      jsonb_build_object('category','Device storage','root_cause','The browser storage quota for this origin is exhausted.','fix','Trim cached payloads, evict old offline records on a TTL, and handle QuotaExceededError gracefully.','team','Frontend Platform')
    WHEN p_msg ILIKE '%cannot read%' OR p_msg ILIKE '%undefined is not%' OR p_msg ILIKE '%null is not%' OR p_msg ILIKE '%is not a function%' THEN
      jsonb_build_object('category','Null reference','root_cause','A component rendered before its data resolved, or an API returned a shape the component did not expect.','fix','Add optional chaining plus loading/empty states, and validate the API response shape at the boundary.','team','Frontend')
    WHEN p_msg ILIKE '%geolocation%' OR p_msg ILIKE '%position%unavailable%' THEN
      jsonb_build_object('category','Device hardware','root_cause','GPS capture failed or timed out on a low-end handset or indoors.','fix','Try high accuracy briefly, then fall back to low-accuracy cellular positioning instead of blocking the flow.','team','Frontend')
    ELSE
      jsonb_build_object('category','Unclassified','root_cause','No signature match - requires stack-trace review against the reported source file and line.','fix','Reproduce on the reported route/browser, capture the source-mapped stack, then triage.','team','Engineering')
  END;
$fn$;

GRANT EXECUTE ON FUNCTION public.cto_classify_error(text) TO authenticated, service_role;
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'suspicious_ips', COALESCE((SELECT jsonb_agg(x) FROM (
        SELECT COALESCE(ip_address::text,'unknown') AS ip, count(*) AS attempts, count(DISTINCT phone) AS distinct_identities, max(created_at) AS last_seen
