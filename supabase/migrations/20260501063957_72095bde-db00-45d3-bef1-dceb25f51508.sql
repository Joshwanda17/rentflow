-- Align create_ledger_transaction's internal solvency check with the
-- system-wide "ledger truth" filter used by wallet_ledger_truth_view,
-- get_user_available_balance, and the user-facing wallet view.
--
-- Without this, admin_correction sweeps and system_balance_correction
-- legs phantom-shrink the solvency cap below the figure the rest of
-- the system trusts (cache, truth view, strict RPC, wallet-deduction
-- pre-check), causing legitimate CFO deductions and reconciliations
-- to fail with "Insufficient ledger balance" even when bulk reconcile
-- reports the wallet is in sync.
--
-- See: mem/constraints/user-facing-ledger-filter.md
--      mem/architecture/wallet-baseline-anchor.md
--      mem/architecture/anchored-cache-drift.md
  ...
  LOOP
    ...
    ELSIF v_entry->>'direction' = 'cash_out' THEN
      ...
      IF NOT skip_balance_check AND COALESCE(v_entry->>'ledger_scope', 'wallet') = 'wallet' THEN
        -- Solvency must use the SAME filter as wallet_ledger_truth_view and
        -- the user-facing wallet helpers: exclude admin_correction sweeps
        -- and system_balance_correction noise. Otherwise historical CFO
        -- corrections phantom-shrink the cap below the figure trusted by
        -- the cache, the truth view, and the wallet-deduction pre-check.
        SELECT COALESCE(SUM(
