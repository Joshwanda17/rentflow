...
BEGIN
  ...
  -- Referential integrity for re-pointed landlord / lc1
  IF _new_landlord IS NULL THEN
    RAISE EXCEPTION 'Landlord is required';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM public.landlords WHERE id = _new_landlord) THEN
    RAISE EXCEPTION 'Selected landlord does not exist';
  END IF;
  IF _new_lc1 IS NOT NULL AND NOT EXISTS (SELECT 1 FROM public.lc1_chairpersons WHERE id = _new_lc1) THEN
    RAISE EXCEPTION 'Selected LC1 chairperson does not exist';
  END IF;
  ...
  -- Per-payment schedule rebuild (delete + repopulate, same as before)
  DELETE FROM public.rent_request_payments WHERE rent_request_id = p_request_id;
  DECLARE
    _per_payment numeric := round(_total / _new_payments);
    _remainder numeric := _total - (_per_payment * _new_payments);
    _i int;
    _days_between int := GREATEST(1, _new_duration / _new_payments);
  BEGIN
    FOR _i IN 1.._new_payments LOOP
      INSERT INTO public.rent_request_payments(rent_request_id, payment_number, amount, due_date)
      VALUES (
        p_request_id,
        _i,
        CASE WHEN _i = _new_payments THEN _per_payment + _remainder ELSE _per_payment END,
        (now()::date + (_i * _days_between))
      );
    END LOOP;
  END;
