...
BEGIN
  ...
  WITH d AS (
    ...
    WHERE gl.category = 'rent_disbursement'
      ...
      AND (gl.source_table IS NULL OR gl.source_table = 'rent_requests')
      AND gl.source_id IS NOT NULL
      AND EXISTS (SELECT 1 FROM rent_requests rq WHERE rq.id = gl.source_id)
  ), j AS (
    SELECT d.*,
    ...
    FROM d
    JOIN rent_requests rr ON rr.id = d.rent_request_id
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
           ...
           'status', COALESCE(NULLIF(request_status,''),'unknown'),
