...
DECLARE
  v_trunc text;
  v_series_start timestamptz;   -- trailing window for the chart
  v_cur_start timestamptz;      -- start of the currently selected period
  v_prev_start timestamptz;     -- start of the immediately preceding period
  v_totals jsonb;
  v_series jsonb;
  v_top jsonb;
  v_invitees jsonb;
...
BEGIN
  ...
  WHERE role='agent' AND coalesce(enabled,true) AND created_at >= v_cur_start;
  ...
  -- trailing series for the growth chart
  WITH buckets AS (
  ...
  ag AS (
    ...
    FROM public.user_roles
    WHERE role='agent' AND coalesce(enabled,true) AND created_at >= v_series_start
    GROUP BY 1
  ),
  sa AS (
    SELECT date_trunc(v_trunc, created_at) AS b, count(*) AS c
