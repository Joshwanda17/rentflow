-- ============================================================================
-- Switch the user-facing cash withdrawal pickup code from WPO-XXXXX to a
-- simple 4-digit numeric code (matches the agent cash-deposit PIN UX).
--
-- The 4-digit space only has 10,000 values, so the previous GLOBAL unique
-- constraint on payout_codes.code can no longer hold over the platform's
-- lifetime. We scope uniqueness to ACTIVE codes only (pending/claimed) — a
-- code value is reusable once its row is paid or expired. Lookups in the
-- approve-withdrawal gate are tied to withdrawal_request_id, and the agent
-- in-person verify filters by status='pending', so reuse over time is safe.
-- ============================================================================

-- 1. Replace the global unique constraint with a partial unique index that
--    only enforces uniqueness while a code is live.
...
-- 2. submit_withdrawal_request — generate a 4-digit code for cash payouts.
...
BEGIN
  ...
  IF v_method = 'cash' THEN
    -- 4-digit numeric pickup code. Uniqueness only needs to hold among
    -- currently-active codes (partial unique index uq_payout_codes_active_code).
    LOOP
...
-- 3. resend_payout_code — issue a fresh 4-digit code on resend.
...
BEGIN
  ...
  -- Issue a fresh 4-digit code (unique among active codes).
  LOOP
