...
BEGIN
  ...
  END IF;
  IF _reason IS NULL OR length(trim(_reason)) < 10 THEN RAISE EXCEPTION 'reason_min_10_chars'; END IF;
  ...
  VALUES (
    ...
    'profiles',
    _target_user::text,
    jsonb_build_object('reason', trim(_reason), 'approved_at', now(), 'target_user_id', _target_user, 'actor_id', v_actor)
  );
END;
$function$;
...
BEGIN
  ...
  VALUES (
    ...
    'profiles',
    _target_user::text,
    jsonb_build_object('reason', trim(_reason), 'rejected_at', now(), 'target_user_id', _target_user, 'actor_id', v_actor)
  );
END;
$function$;