...
DECLARE
  ...
  v_r text;
BEGIN
  -- Record IP for logs only (no longer used to block).
  BEGIN
    v_ip_txt := split_part(
  ...
  -- Normalize the user-agent (trim + lower) so trivial casing differences don't bypass the check.
  v_ua_norm := nullif(btrim(lower(coalesce(p_user_agent, ''))), '');

  IF NOT v_is_staff AND NOT v_is_exempt_path THEN
    -- Rule 1: same device fingerprint in the last 24h.
    ...
    -- Rule 2 (new): same browser user-agent in the last 24h, even if the fingerprint was cleared.
...
-- Support index for the new user-agent lookup.
CREATE INDEX IF NOT EXISTS idx_signup_attempts_ua_time
  ON public.signup_attempts (lower(btrim(user_agent)), created_at DESC)
