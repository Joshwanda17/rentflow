...
BEGIN
  ...
  -- No active tenants OR nothing expected today → no restriction, agent may withdraw freely.
  IF COALESCE(v_perf.expected_daily, 0) <= 0 OR COALESCE(v_perf.active_count, 0) <= 0 THEN
    RETURN NEW;
  END IF;

  -- v_agent_daily_eligibility.today_pct is a 0-1 fraction; convert to percent for the 20% rule.
