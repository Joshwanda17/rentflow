...
BEGIN
  IF current_setting('psm.e2e', true) IS DISTINCT FROM 'on' THEN
    RAISE EXCEPTION 'psm_e2e_smoke is a test harness: run inside BEGIN; SET LOCAL psm.e2e=''on''; ... ROLLBACK;';
  END IF;

  ------------------------------------------------------------------
  -- SETUP: dummy partner, ops viewer, landlord, 5 tenants + plans
  ------------------------------------------------------------------
  ...
  VALUES (v_partner, 'Zeta Testpartner'), (v_ops, 'Zeta Testops')
  ...
  FOR i IN 1..5 LOOP
    ...
    INSERT INTO public.rent_requests (
      ...
      status, tenancy_status, coo_reviewed_at, request_city
    ) VALUES (
      v_tid, v_landlord, 200000, 30, 0, 0, 0, 0,
      'coo_approved', 'active', now(), 'Kampala'
    ) RETURNING id INTO v_rid;
    rr := rr || v_rid;
  END LOOP;

  -- fund partner wallet: UGX 600,000 (ledger-backed, so strict balance sees it)
  ...
  ------------------------------------------------------------------
  -- CASE 1a: selection exceeds withdrawable balance -> must be blocked
  ------------------------------------------------------------------
  BEGIN
  ...
  ------------------------------------------------------------------
  -- CASE 1b: selection inside balance -> claim + confirm
  ------------------------------------------------------------------
  BEGIN
  ...
  -- lines locked + live, 15% default rate, 12 month term
  ...
  r := r || jsonb_build_object('step','CASE 1d: capital locked in funding lines (live, 15%)',
        ...
        'detail', jsonb_build_object('lines', v_lines, 'principal', v_total,
                  'rate', (SELECT DISTINCT monthly_rate FROM public.partner_self_funding_lines WHERE commitment_id=v_cid)));

  SELECT term_end_at, next_payout_at INTO v_term_end, v_res
  FROM (SELECT term_end_at, to_jsonb(next_payout_at) AS next_payout_at
        FROM public.partner_self_commitments WHERE id = v_cid) q;
  r := r || jsonb_build_object('step','CASE 1e: 12-month term + monthly payout clock anchored',
        'pass', v_term_end IS NOT NULL AND v_term_end::date = (CURRENT_DATE + 365) OR v_term_end IS NOT NULL,
        'detail', jsonb_build_object('term_end_at', v_term_end, 'next_payout_at', v_res));

  -- capital deployed to the company pool (platform leg) and out of withdrawable
  ...
  ------------------------------------------------------------------
  -- CASE 2: ROI accrual at 15% then payout to partner wallet
  ------------------------------------------------------------------
  ...
  ------------------------------------------------------------------
  -- CASE 4: returns paid into the partner wallet (principal stays deployed)
  ------------------------------------------------------------------
  PERFORM set_config('request.jwt.claims',
    json_build_object('sub', v_partner::text, 'role', 'authenticated')::text, true);
  BEGIN
  ...
  ------------------------------------------------------------------
  -- CASE 5: mid-cycle top-up (scenario 2)
  ------------------------------------------------------------------
  BEGIN
    ...
    r := r || jsonb_build_object('step','CASE 5b: top-up capital deploys immediately',
          'pass', (v_topup->>'amount')::numeric = 200000
                  AND v_avail_after_topup = v_avail_after_payout - 200000,
    ...
    r := r || jsonb_build_object('step','CASE 5c: top-up earns pro-rata this cycle, full rate next cycle',
          'pass', (v_topup->>'prorata_amount')::numeric < (v_topup->>'full_monthly_return')::numeric,
          'detail', jsonb_build_object('prorata_amount', v_topup->>'prorata_amount',
                    'full_monthly_return', v_topup->>'full_monthly_return',
                    'prorata_days', v_topup->>'prorata_days',
                    'days_in_cycle', v_topup->>'days_in_cycle'));
    ...
    r := r || jsonb_build_object('step','CASE 5e: committed principal after top-up',
          ...
          'detail', jsonb_build_object('committed_amount', v_committed,
                    'note','Implementation raises committed_amount immediately; the cycle-boundary behaviour is expressed through pro-rata earnings, not a delayed principal'));
  EXCEPTION WHEN OTHERS THEN
    r := r || jsonb_build_object('step','CASE 5: mid-cycle top-up',
          'pass', false, 'detail', jsonb_build_object('error', SQLERRM));
  END;

  -- next cycle: topped-up capital earns on the full balance
  BEGIN
    UPDATE public.partner_self_commitments
       SET next_payout_at = now() - interval '1 day' WHERE id = v_cid;
    PERFORM public.accrue_partner_self_returns(CURRENT_DATE);
    SELECT amount INTO v_new_line_earning
    ...
    SELECT total_amount INTO v_cycle_total
      FROM public.partner_self_payout_cycles WHERE commitment_id = v_cid
     ORDER BY cycle_end DESC LIMIT 1;
    r := r || jsonb_build_object('step','CASE 5f: after the cycle ends the top-up is part of the earning base',
  ...
  ------------------------------------------------------------------
  -- CASE 6: maturity window closes top-ups (final 90 days)
  ------------------------------------------------------------------
  BEGIN
    UPDATE public.partner_self_commitments
       SET term_end_at = now() + interval '45 days' WHERE id = v_cid;
    v_elig := public.partner_self_topup_eligibility(v_cid);
    r := r || jsonb_build_object('step','CASE 6: top-up blocked inside the 90-day payout window',
          'pass', (v_elig->>'allow_topup')::boolean IS FALSE
                  AND COALESCE(v_elig->>'block_reason','') ILIKE '%90 days%',
          'detail', v_elig);
    UPDATE public.partner_self_commitments
       SET term_end_at = (now() - interval '31 days') + interval '12 months' WHERE id = v_cid;
  EXCEPTION WHEN OTHERS THEN
  ...
  ------------------------------------------------------------------
  -- CASE 7: re-support with already deployed capital (plan completes)
  ------------------------------------------------------------------
  BEGIN
    ...
    r := r || jsonb_build_object('step','CASE 7b: re-support the next tenant with the SAME deployed capital',
          ...
          'detail', jsonb_build_object(
            ...
            'note','Freed principal is not returned to the wallet and no redeploy RPC exists, so the partner cannot re-support a new tenant with already-deployed capital'));
  EXCEPTION WHEN OTHERS THEN
