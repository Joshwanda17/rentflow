...
BEGIN
  ...
  suggested AS (
    SELECT f.*,
      -- CONSERVATIVE, repayment-driven suggestion.
      -- Philosophy: begin low, grow with proven repayment & activity.
      --   * base_potential: score curved against a modest 5M ceiling (not 30M).
      --   * trust_multiplier gates that potential by real advance track record:
      --       - brand new (0 advances taken)  -> only 12% of potential (a starter)
      --       - has taken advances            -> 0.35 + 0.65*repay_rate, and a
      --                                           small bump for each fully repaid
      --                                           advance (rewards paying early/often)
      --   * result rounded to 50k, floored at 100k, hard-capped at 5M.
      LEAST(5000000,
        GREATEST(100000,
          ROUND(
            (
              power(LEAST(GREATEST(f.total_score, 0), 100) / 100.0, 1.5) * 5000000
              * CASE
                  WHEN f.advances_count = 0 THEN 0.12
                  ELSE LEAST(1.0,
                         0.35
                         + 0.55 * COALESCE(f.repay_rate, 0)
