...
BEGIN
  ...
  -- Total platform commission is always 10% of the allocated amount.
  v_total_commission := round(p_amount * 0.10, 2);

  -- If the allocating agent was recruited (is a sub-agent), their recruiter
  -- (parent agent) earns a 2% override and the sub-agent keeps 8%.
  ...
  v_legs := jsonb_build_array(
    ...
    jsonb_build_object('user_id',p_agent_id,'amount',v_commission_earned,'direction','cash_in','category','agent_commission_earned','ledger_scope','wallet','classification','production','description',CASE WHEN v_parent_agent_id IS NOT NULL THEN '8% commission on float allocation (sub-agent share)' ELSE '10% commission on float allocation' END,'recipient_type','user','source_table','agent_collections','source_id',p_rent_request_id),
    jsonb_build_object('user_id',p_agent_id,'amount',v_total_commission,'direction','cash_out','category','agent_commission_payable','ledger_scope','platform','classification','production','description','Platform commission payout (10%)','source_table','agent_collections','source_id',p_rent_request_id)
  );

  -- Add the recruiter's 2% override leg when the allocating agent is a sub-agent.
  IF v_parent_agent_id IS NOT NULL AND v_parent_override > 0 THEN
    v_legs := v_legs || jsonb_build_array(
      jsonb_build_object('user_id',v_parent_agent_id,'amount',v_parent_override,'direction','cash_in','category','agent_commission_earned','ledger_scope','wallet','classification','production','description','2% recruiter override on sub-agent float allocation','recipient_type','user','linked_party',p_agent_id,'source_table','agent_collections','source_id',p_rent_request_id)
    );
  END IF;
  ...
  VALUES (p_agent_id, p_tenant_id, p_rent_request_id, p_amount, 'cash', v_tracking_id, p_notes, v_float_balance, v_float_balance - p_amount)
  ...
  INSERT INTO public.agent_allocation_traces (
    ...
    v_outstanding, v_outstanding - p_amount, v_float_balance, v_float_balance - p_amount,
