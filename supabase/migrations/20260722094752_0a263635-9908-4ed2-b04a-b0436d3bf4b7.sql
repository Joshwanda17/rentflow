...
BEGIN
  FOR r IN
    ...
    WHERE user_id = 'ebf0897b-dfdf-4403-ad5c-1c988c72e67c'
    ...
    PERFORM public.create_ledger_transaction(
      jsonb_build_array(
        jsonb_build_object(
          ...
          'source_id', r.id::text,
          'description', 'Refund: parent-agent listing rejection penalty (KANUNA KEITH) — link was unstable',
          'currency', 'UGX'
        ),
        jsonb_build_object(
          'amount', 6000,
          ...
          'source_table', 'general_ledger',
          'source_id', r.id::text,
          'description', 'Reversal of parent-agent rejection penalty for Watsala Enock',
          'currency', 'UGX'
        )
      ),
      'refund_parent_rejection_penalty:' || r.id::text,
      true
    );
  ...
  VALUES (
    'ebf0897b-dfdf-4403-ad5c-1c988c72e67c',
    'parent_rejection_penalty_refund',
    'general_ledger',
    'ebf0897b-dfdf-4403-ad5c-1c988c72e67c',
    jsonb_build_object(
      'refunded_count', v_count,
      'refunded_amount_ugx', v_total,
      'sub_agent', 'KANUNA KEITH',
      'reason', 'ParntPenRfd'
    )
  );
END $$;
...
BEGIN
  ...
  IF v_listing.agent_id IS NOT NULL THEN
    ...
    VALUES (
      ...
      'Your listing "' || v_listing_title || '" was rejected by ' || COALESCE(v_caller_name, 'Landlord Ops') ||
        '. Reason: ' || v_reason || '. A UGX 2,000 charge was applied to your wallet for this rejection.',
      'warning',
      jsonb_build_object('listing_id', v_listing.id, 'listing_title', v_listing.title, 'reason', v_reason,
        'rejected_by', v_caller, 'rejected_by_name', v_caller_name, 'action', 'listing_rejected', 'charge', 2000)
    );

    BEGIN
      PERFORM public.create_ledger_transaction(
        jsonb_build_array(
          jsonb_build_object('user_id', v_listing.agent_id, 'amount', 2000, 'direction', 'cash_out',
          ...
          jsonb_build_object('amount', 2000, 'direction', 'cash_in', 'category', 'listing_rejection_recovery',
      ...
      IF NOT v_parent_is_merchant THEN
        ...
        SELECT COUNT(*) INTO v_recent_parent_charge
         ...
         WHERE user_id = v_parent_id
           AND category = 'listing_rejection_penalty'
           AND amount = 6000 AND direction = 'cash_out'
        ...
        THEN
          BEGIN
            PERFORM public.create_ledger_transaction(
              jsonb_build_array(
                jsonb_build_object('user_id', v_parent_id, 'amount', 6000, 'direction', 'cash_out',
                  'category', 'listing_rejection_penalty', 'ledger_scope', 'wallet', 'wallet_bucket', 'withdrawable',
                  'source_table', 'house_listings', 'source_id', p_listing_id::text,
                  'description', 'Parent-agent penalty: sub-agent ' || v_sub_name || ' had 3 listings rejected',
                  'currency', 'UGX'),
                jsonb_build_object('amount', 6000, 'direction', 'cash_in', 'category', 'listing_rejection_recovery',
                  'ledger_scope', 'platform', 'source_table', 'house_listings', 'source_id', p_listing_id::text,
        ...
        VALUES (
          ...
          CASE WHEN v_parent_charged
               THEN '⚠️ Sub-agent listing rejected — UGX 6,000 charge'
               ELSE '⚠️ Sub-agent listing rejected'
          END,
          'Your sub-agent ' || v_sub_name || ' had a listing rejected ("' || v_listing_title ||
            '"). Reason: ' || v_reason ||
            CASE WHEN v_parent_charged
                 THEN '. A UGX 6,000 charge was applied to your wallet (every 3rd rejection).'
                 ELSE '' END,
          'warning',
  ...
  RETURN jsonb_build_object('ok', true, 'listing_id', v_listing.id,
    ...
    'charge_amount', CASE WHEN v_charged THEN 2000 ELSE 0 END, 'auto_blocked', false,
