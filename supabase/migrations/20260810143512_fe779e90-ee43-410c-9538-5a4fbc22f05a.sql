CREATE OR REPLACE FUNCTION public.get_partner_ops_daily_report(p_date date DEFAULT ((now() AT TIME ZONE 'Africa/Nairobi')::date))
RETURNS jsonb
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'paid_today', coalesce((
      SELECT jsonb_agg(x ORDER BY x->>'roi' DESC) FROM (
        SELECT jsonb_build_object(
          'name', coalesce(p.full_name, 'Partner'),
          'phone', p.phone,
          'roi', gl.amount,
          'principal', coalesce((SELECT sum(ip.investment_amount) FROM investor_portfolios ip WHERE ip.investor_id = gl.user_id AND ip.status = 'active'),0),
          'at', gl.created_at
        ) AS x
    ...
    'pending_portfolios', (
      SELECT jsonb_build_object(
        ...
        'pending_redemption_requests', (SELECT count(*) FROM portfolio_action_requests WHERE status = 'pending' AND request_type = 'REDEMPTION_REQUEST'),
        'rows', coalesce((
          SELECT jsonb_agg(jsonb_build_object(
            'name', coalesce(p.full_name,'Partner'),
            'portfolio_code', ip.portfolio_code,
    ...
    'promissory_notes', (
      SELECT jsonb_build_object(
        'created_today', (SELECT count(*) FROM promissory_notes WHERE created_at >= d0 AND created_at < d1),
        'created_today_amount', (SELECT coalesce(sum(amount),0) FROM promissory_notes WHERE created_at >= d0 AND created_at < d1),
        ...
        'verified_count', (SELECT count(*) FROM promissory_notes WHERE status = 'activated'),
        'verified_amount', (SELECT coalesce(sum(amount),0) FROM promissory_notes WHERE status = 'activated'),
        'verified_today', (SELECT count(*) FROM promissory_notes WHERE status = 'activated' AND approved_at >= d0 AND approved_at < d1),
        'rows', coalesce((
          SELECT jsonb_agg(jsonb_build_object(
            'partner_name', n.partner_name,
            'agent_name', coalesce(p.full_name,'Agent'),
            'amount', n.amount,
            'status', n.status,
            'at', n.created_at
          ...
          FROM (
            ...
            WHERE (created_at >= w0 AND created_at < d1) OR status = 'pending'
    ...
    'withdrawals', (
      SELECT jsonb_build_object(
        'completed_today_count', (
          ...
          WHERE w.status = 'completed' AND coalesce(w.processed_at, w.updated_at) >= d0 AND coalesce(w.processed_at, w.updated_at) < d1
            AND EXISTS (SELECT 1 FROM investor_portfolios ip WHERE ip.investor_id = w.user_id)
        ),
        'completed_today_amount', (
          SELECT coalesce(sum(w.amount),0) FROM withdrawal_requests w
          WHERE w.status = 'completed' AND coalesce(w.processed_at, w.updated_at) >= d0 AND coalesce(w.processed_at, w.updated_at) < d1
            AND EXISTS (SELECT 1 FROM investor_portfolios ip WHERE ip.investor_id = w.user_id)
        ),
        'rows', coalesce((
          SELECT jsonb_agg(jsonb_build_object(
            'name', coalesce(p.full_name,'Partner'),
            'amount', w.amount,
            'method', coalesce(w.payout_method, w.fin_ops_payment_method, 'mobile_money'),
            'at', coalesce(w.processed_at, w.updated_at)
...
REVOKE ALL ON FUNCTION public.get_partner_ops_daily_report(date) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_partner_ops_daily_report(date) TO authenticated, service_role;