...
WITH resolved AS (
  ...
  -- Signed trial-balance movement per cash account (debits less credits), the
  -- identical basis the Statement of Financial Position uses for A1 and A2.
  -- Published so the closing cash figure can be tied to the Balance Sheet
  -- without re-deriving signs by hand.
...
SELECT jsonb_build_object(
  ...
  'to', p_to,
  'currency', 'UGX',
  'cash_definition', 'Cash and cash equivalents = Cash and Bank Balances (A1) plus Cash at Hand — Float with Agents (A2), each carried at its signed General Ledger trial-balance value (debits less credits). Transfers between the two cash accounts are eliminated.',
...
REVOKE ALL ON FUNCTION public.get_statement_of_cash_flows(timestamp with time zone, timestamp with time zone) FROM anon;
