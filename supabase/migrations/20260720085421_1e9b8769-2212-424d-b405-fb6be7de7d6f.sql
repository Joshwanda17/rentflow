  ...
  LOOP
    ...
    v_group_id := public.create_ledger_transaction(
      jsonb_build_array(
        ...
        jsonb_build_object(
          ...
          'source_table', 'referrals',
          'source_id', r.id::text,
          'description', 'Referral bonus — your invite signed up',
          'ledger_scope', 'wallet',
          'recipient_type', 'user'
        )
      ),
    ...
    BEGIN
      ...
      VALUES (
        ...
        'Someone just signed up using your referral link. UGX '
...
-- Credit Timothy's pending referral now
SELECT public.try_credit_qualified_referrals('94edf49f-83b3-4f4c-b488-d0020166f658'::uuid);
