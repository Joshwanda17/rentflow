...
SELECT r.id,
       ...
       p.phone,
       r.principal,
       -- approved if it reached an approved/paid state, or money actually moved
       CASE
         WHEN r.status IN ('cfo_approved','cfo_paid') OR r.cfo_paid_at IS NOT NULL OR l.credited_at IS NOT NULL
           THEN r.principal
         ELSE NULL
       END,
       -- paid = real disbursement happened (even if later voided/rejected)
       CASE
       ...
       r.created_at,
       -- approval date: suppress stale approval stamps on requests that were rejected and never paid
       CASE
...
SELECT d.id,
       ...
       -- draws that were actually disbursed are approved, even if predating the CFO approval step
       CASE
         WHEN d.cfo_approved_at IS NOT NULL OR d.started_at IS NOT NULL OR cl.credited_at IS NOT NULL
           THEN d.amount
         ELSE NULL
       END,
       CASE
         WHEN d.started_at IS NOT NULL OR cl.credited_at IS NOT NULL
       ...
       CASE
         WHEN d.status IN ('rejected','cfo_rejected') AND d.cfo_approved_at IS NULL THEN NULL
       ...
       COALESCE(d.started_at, cl.credited_at),
       d.status,
       cl.txn_ref,
