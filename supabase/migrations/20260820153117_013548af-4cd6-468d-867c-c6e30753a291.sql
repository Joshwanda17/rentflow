...
BEGIN
  ...
  v_is_finance := public.has_role(auth.uid(), 'cfo')
    ...
    -- Trusted server-side callers (scheduled Merchant Float Morning Report)
    -- have no auth.uid(); they must read the SAME figures as the board.
  ...
  unbacked AS (
    ...
    LEFT JOIN public.merchant_out_of_pocket_advances o
     ...
     -- LEDGER TRUTH: a claim only counts as money we owe when the desk's float
     -- position, reconstructed at that payout's own timestamp, was negative.
