...
BEGIN
  IF public.has_role(p_user_id, 'agent') THEN
    ...
    v_subagents_bonus := (v_registered_subagents * 60000) + (v_active_subagents * 60000);

    SELECT COUNT(DISTINCT rr.tenant_id) INTO v_tenants_registered
    FROM public.rent_requests rr
    WHERE rr.agent_id = p_user_id
      AND rr.tenant_id IS NOT NULL
      AND rr.agent_id <> rr.tenant_id;
    v_rent_history_bonus := LEAST(v_tenants_registered * 100000, 5000000);

    SELECT COALESCE(SUM(amount), 0) INTO v_collections_total
    FROM public.agent_collections WHERE agent_id = p_user_id;
    v_agent_allocations_bonus := LEAST(v_collections_total * 0.35, 8000000);
    ...
    v_partners_bonus := LEAST(v_promissory_count * 40000, 2000000);
...
RETURNS TABLE (
  ...
  houses_listed bigint, rent_requests bigint,
  tenants_registered bigint, promissory_notes bigint,
  base_limit numeric, subagents_bonus numeric, tenants_bonus numeric,
  collections_bonus numeric, promissory_bonus numeric,
  houses_bonus numeric, requests_bonus numeric,
  total_limit numeric, stored_total_limit numeric, total_matched bigint
)
...
BEGIN
  ...
  computed AS (
    SELECT m.*, 30000::numeric AS c_base,
      ((m.registered_subs * 60000) + (m.active_subs * 60000))::numeric AS c_subs,
      LEAST(m.tenants_registered * 100000, 5000000)::numeric AS c_tenants,
      LEAST(m.rent_collected * 0.35, 8000000)::numeric AS c_coll,
      LEAST(m.promissory_notes * 40000, 2000000)::numeric AS c_promis,
