...
AS $$
  ...
  UNION ALL
  SELECT 'district', btrim(s.district), count(*)
  FROM scoped s WHERE btrim(coalesce(s.district, '')) <> '' GROUP BY 2
  UNION ALL
  SELECT 'subcounty', btrim(s.sub_county), count(*)
  FROM scoped s WHERE btrim(coalesce(s.sub_county, '')) <> '' GROUP BY 2
  UNION ALL
  SELECT 'village', btrim(s.village), count(*)
  FROM scoped s WHERE btrim(coalesce(s.village, '')) <> '' GROUP BY 2
  UNION ALL
  SELECT 'category', s.house_category, count(*)
  FROM scoped s WHERE btrim(coalesce(s.house_category, '')) <> '' GROUP BY 2
$$;

GRANT EXECUTE ON FUNCTION public.get_house_listing_filter_options(text, text, text) TO anon, authenticated, service_role;