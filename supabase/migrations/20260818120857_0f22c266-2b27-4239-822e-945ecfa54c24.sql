CREATE OR REPLACE FUNCTION public.get_partner_total_trend(p_preset text DEFAULT 'today'::text)
 RETURNS jsonb
 LANGUAGE plpgsql
...
begin
  ...
  create temporary table if not exists _pt_partners (user_id uuid, joined_at timestamp) on commit drop;

  with partners as (
    select p.investor_id as user_id,
           min(p.created_at at time zone 'Africa/Kampala') as joined_at
    from investor_portfolios p
    where p.investor_id is not null
      and coalesce(p.status, '') not in ('cancelled')
  ...
  from (
    select p.investor_id, min(p.created_at at time zone 'Africa/Kampala') as joined_at
    from investor_portfolios p
    where p.investor_id is not null and coalesce(p.status, '') not in ('cancelled')
    group by p.investor_id
  ) q
  where q.joined_at < v_start;

  select count(distinct p.investor_id) into v_total_partners
  from investor_portfolios p
  where p.investor_id is not null and coalesce(p.status, '') not in ('cancelled');
