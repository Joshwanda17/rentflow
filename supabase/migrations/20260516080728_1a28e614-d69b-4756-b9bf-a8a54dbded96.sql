...
CREATE TABLE IF NOT EXISTS public.agent_allocation_traces (
...
CREATE INDEX IF NOT EXISTS idx_aat_agent ON public.agent_allocation_traces(agent_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_aat_rr    ON public.agent_allocation_traces(rent_request_id);
CREATE INDEX IF NOT EXISTS idx_aat_txn   ON public.agent_allocation_traces(transaction_group);

ALTER TABLE public.agent_allocation_traces ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "agent reads own traces" ON public.agent_allocation_traces;
CREATE POLICY "agent reads own traces"
  ON public.agent_allocation_traces FOR SELECT
  USING (agent_id = auth.uid());

DROP POLICY IF EXISTS "staff reads all traces" ON public.agent_allocation_traces;
CREATE POLICY "staff reads all traces"
  ON public.agent_allocation_traces FOR SELECT
  USING (
...
BEGIN
  ...
  RAISE NOTICE 'agent_allocate_tenant_payment trace: %', jsonb_build_object(
