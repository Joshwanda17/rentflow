-- 1) Fix recalculate_credit_limit: only count sub-agents that themselves qualify as agents
...
BEGIN
  IF public.has_role(p_user_id, 'agent') THEN
    -- Only count sub-agents who themselves qualify as agents (behavior-based).
    -- Merely being referred/invited does NOT make someone a sub-agent.
...
-- 2) Fix get_agent_advance_limits: sub-agent counts filtered by the qualifying set
...
BEGIN
  ...
  sub_direct AS (
    -- Only sub-agents who themselves qualify as agents are counted.
...
-- 3) Recalculate stored limits for all agents whose sub-agent counts could change
DO $$
DECLARE
  r RECORD;
BEGIN
  FOR r IN
    SELECT DISTINCT uid FROM (
      SELECT parent_agent_id AS uid FROM public.agent_subagents
        WHERE parent_agent_id IS NOT NULL AND sub_agent_id IS NOT NULL
      UNION
      SELECT user_id AS uid FROM public.credit_access_limits
        WHERE bonus_from_subagents > 0
    ) s
    WHERE uid IS NOT NULL
  LOOP
    PERFORM public.recalculate_credit_limit(r.uid);
  END LOOP;
END $$;