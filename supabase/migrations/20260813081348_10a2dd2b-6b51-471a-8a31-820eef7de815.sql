-- =========================================================
-- 1. ROOT-CAUSE FIX: tenant_ops staff could only read
--    rent_requests through the agent policy
--    (agent_verified = false AND status IN ('pending','approved'))
--    which returns exactly 172 rows -> "172 tenants".
--    Give tenant_ops the same read visibility other ops roles have.
-- =========================================================
CREATE POLICY "Tenant ops can view all rent requests"
ON public.rent_requests FOR SELECT TO authenticated
USING (public.has_role(auth.uid(), 'tenant_ops'));

CREATE POLICY "Tenant ops can view all profiles"
ON public.profiles FOR SELECT TO authenticated
USING (public.has_role(auth.uid(), 'tenant_ops'));

CREATE POLICY "Tenant ops can view all agent_collections"
ON public.agent_collections FOR SELECT TO authenticated
USING (public.has_role(auth.uid(), 'tenant_ops'));

CREATE POLICY "Tenant ops can view landlord payouts"
ON public.landlord_payouts FOR SELECT TO authenticated
USING (public.has_role(auth.uid(), 'tenant_ops'));

-- =========================================================
-- 2. Headline report (server-side aggregation, EAT windows)
-- =========================================================
...
REVOKE ALL ON FUNCTION public.ops_tenant_products_services_report(date, date) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.ops_tenant_products_services_report(date, date) TO authenticated, service_role;

-- =========================================================
-- 3. Tenant rows for the report table (server-side filters)
-- =========================================================
...
BEGIN
  ...
  base AS (
    ...
    WHERE (
      -- tenants relevant to the reporting period
...
REVOKE ALL ON FUNCTION public.ops_tenant_products_services_rows(date, date, text, text, uuid, text, text, int, int) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.ops_tenant_products_services_rows(date, date, text, text, uuid, text, text, int, int) TO authenticated, service_role;