...
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      v_deduct := LEAST(v_avail, v_adv.outstanding_balance);
...
-- Schedule the sweep every 15 minutes so withdrawable funds are auto-debited
-- toward outstanding advances continuously, not just on the daily cycle.
DO $cron$
BEGIN
...
-- Settle the current backlog immediately.
SELECT public.sweep_agent_advance_recovery();