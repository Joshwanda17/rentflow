...
BEGIN
  ...
  -- bucket "new agents" over time. Held in an array for reuse.
  WITH qa AS (
  ...
  SELECT
    ...
    count(*) FILTER (WHERE created_at >= v_prev_start AND created_at < v_cur_start)
  INTO v_total_agents, v_new_agents, v_prev_agents
  FROM qa;
