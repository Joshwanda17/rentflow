...
BEGIN
  ...
  -- Idempotency key now includes a per-attempt nonce so repeat payments of the
  -- same amount on the same tenant/rent_request are NOT silently short-circuited
  -- by create_ledger_transaction. Each call posts its own ledger transaction,
  -- so float decreases, commission credits, and landlord receives the leg.
