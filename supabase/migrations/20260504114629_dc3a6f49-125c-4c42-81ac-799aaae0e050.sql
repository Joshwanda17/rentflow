...
BEGIN
    ...
    PERFORM public.create_ledger_transaction(
      ...
      p_entries => jsonb_build_array(
        jsonb_build_object(
          ...
          'ledger_scope',  'wallet',
          'description',   format('Self-heal cache drift before allocation (delta=%s)', v_cache_delta)
        ),
        jsonb_build_object(
          'user_id',       p_agent_id,
          'amount',        v_cache_delta,
          ...
          'description',   format('Platform leg for self-heal (delta=%s)', v_cache_delta)
        )
      )
    );
  ...
  -- The withdrawable bucket on `wallets` IS the commission bucket. It is
  -- the only authoritative "locked commission" figure: every credit and
  -- debit (withdrawals, transfers, corrections, etc.) flows through it
  -- via apply_wallet_movement. Re-summing categories here drifts because
  -- it can't keep up with every debit path.
  SELECT GREATEST(0, COALESCE(withdrawable_balance, 0))
    INTO v_commission
    FROM public.wallets
  ...
  -- ----- Float debit (wallet) + cash to landlord (platform) -----
  INSERT INTO public.general_ledger (
    ...
    format('Float used for tenant payment — %s', v_tracking_id), 'wallet', v_txn_group
  );
  ...
  IF v_post_balance IS NOT NULL AND v_post_balance < 0 THEN
    INSERT INTO public.wallet_commission_drift (
      ...
      p_agent_id, v_commission, v_total_wallet, p_amount,
      'agent_allocate_tenant_payment_post_negative_balance'
    );
  END IF;
