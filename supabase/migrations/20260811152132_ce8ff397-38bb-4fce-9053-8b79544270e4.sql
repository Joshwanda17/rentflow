...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'topups', (
      SELECT jsonb_build_object(
        'requested_count', (SELECT count(*) FROM pending_wallet_operations WHERE category = 'pending_portfolio_topup' AND operation_type = 'portfolio_topup' AND created_at >= d0 AND created_at < d1),
        'requested_amount', (SELECT coalesce(sum(amount),0) FROM pending_wallet_operations WHERE category = 'pending_portfolio_topup' AND operation_type = 'portfolio_topup' AND created_at >= d0 AND created_at < d1),
        'applied_count', (SELECT count(*) FROM pending_wallet_operations WHERE category = 'pending_portfolio_topup' AND operation_type = 'portfolio_topup' AND status = 'completed' AND coalesce(reviewed_at, updated_at) >= d0 AND coalesce(reviewed_at, updated_at) < d1),
        'applied_amount', (SELECT coalesce(sum(amount),0) FROM pending_wallet_operations WHERE category = 'pending_portfolio_topup' AND operation_type = 'portfolio_topup' AND status = 'completed' AND coalesce(reviewed_at, updated_at) >= d0 AND coalesce(reviewed_at, updated_at) < d1),
        'rejected_count', (SELECT count(*) FROM pending_wallet_operations WHERE category = 'pending_portfolio_topup' AND operation_type = 'portfolio_topup' AND status IN ('rejected','failed') AND coalesce(reviewed_at, updated_at) >= d0 AND coalesce(reviewed_at, updated_at) < d1),
        'rejected_amount', (SELECT coalesce(sum(amount),0) FROM pending_wallet_operations WHERE category = 'pending_portfolio_topup' AND operation_type = 'portfolio_topup' AND status IN ('rejected','failed') AND coalesce(reviewed_at, updated_at) >= d0 AND coalesce(reviewed_at, updated_at) < d1),
        'cancelled_count', (SELECT count(*) FROM pending_wallet_operations WHERE category = 'pending_portfolio_topup' AND operation_type = 'portfolio_topup' AND status = 'cancelled' AND coalesce(reviewed_at, updated_at) >= d0 AND coalesce(reviewed_at, updated_at) < d1),
        'cancelled_amount', (SELECT coalesce(sum(amount),0) FROM pending_wallet_operations WHERE category = 'pending_portfolio_topup' AND operation_type = 'portfolio_topup' AND status = 'cancelled' AND coalesce(reviewed_at, updated_at) >= d0 AND coalesce(reviewed_at, updated_at) < d1),
        ...
        'backlog_count', (SELECT count(*) FROM pending_wallet_operations WHERE category = 'pending_portfolio_topup' AND operation_type = 'portfolio_topup' AND status IN ('pending','approved','awaiting_verification')),
        'backlog_amount', (SELECT coalesce(sum(amount),0) FROM pending_wallet_operations WHERE category = 'pending_portfolio_topup' AND operation_type = 'portfolio_topup' AND status IN ('pending','approved','awaiting_verification')),
        'backlog_oldest_days', (SELECT coalesce(max(extract(day FROM (now() - created_at)))::int, 0) FROM pending_wallet_operations WHERE category = 'pending_portfolio_topup' AND operation_type = 'portfolio_topup' AND status IN ('pending','approved','awaiting_verification')),
        'by_status', coalesce((
          ...
          FROM (
            ...
            WHERE category = 'pending_portfolio_topup'
              AND operation_type = 'portfolio_topup'
              AND ((created_at >= d0 AND created_at < d1) OR (coalesce(reviewed_at, updated_at) >= d0 AND coalesce(reviewed_at, updated_at) < d1))
            GROUP BY status
          ) s
        ), '[]'::jsonb)
      )
    ...
    'series', coalesce((
      SELECT jsonb_agg(jsonb_build_object(
        ...
        'topups_applied', (SELECT coalesce(sum(o.amount),0) FROM pending_wallet_operations o WHERE o.category = 'pending_portfolio_topup' AND o.operation_type = 'portfolio_topup' AND o.status = 'completed' AND coalesce(o.reviewed_at, o.updated_at) >= (g.day::date::timestamp AT TIME ZONE 'Africa/Nairobi') AND coalesce(o.reviewed_at, o.updated_at) < ((g.day::date + 1)::timestamp AT TIME ZONE 'Africa/Nairobi')),
        'topups_requested', (SELECT coalesce(sum(o.amount),0) FROM pending_wallet_operations o WHERE o.category = 'pending_portfolio_topup' AND o.operation_type = 'portfolio_topup' AND o.created_at >= (g.day::date::timestamp AT TIME ZONE 'Africa/Nairobi') AND o.created_at < ((g.day::date + 1)::timestamp AT TIME ZONE 'Africa/Nairobi')),
        'withdrawals_completed', (SELECT coalesce(sum(w.amount),0) FROM withdrawal_requests w WHERE w.status = 'completed' AND w.linked_party IS NOT NULL AND w.updated_at >= (g.day::date::timestamp AT TIME ZONE 'Africa/Nairobi') AND w.updated_at < ((g.day::date + 1)::timestamp AT TIME ZONE 'Africa/Nairobi'))
      ) ORDER BY g.day)
      ...
      SELECT jsonb_build_object(
        'days', coalesce((
          SELECT jsonb_agg(jsonb_build_object(
            ...
            'portfolios', (SELECT count(*) FROM investor_portfolios ip WHERE ip.status = 'active' AND ip.next_roi_date::date = f.day::date),
            'total_amount', (SELECT coalesce(sum(round(ip.investment_amount * coalesce(ip.roi_percentage,15) / 100.0)),0) FROM investor_portfolios ip WHERE ip.status = 'active' AND ip.next_roi_date::date = f.day::date)
          ) ORDER BY f.day)
          FROM generate_series(p_end + 1, p_end + 7, interval '1 day') AS f(day)
        ), '[]'::jsonb),
        'weekdays_total', (SELECT coalesce(sum(round(ip.investment_amount * coalesce(ip.roi_percentage,15) / 100.0)),0) FROM investor_portfolios ip WHERE ip.status = 'active' AND ip.next_roi_date::date BETWEEN (p_end + 1) AND (p_end + 7) AND extract(isodow FROM ip.next_roi_date::date) NOT IN (6,7)),
        'weekdays_count', (SELECT count(*) FROM investor_portfolios ip WHERE ip.status = 'active' AND ip.next_roi_date::date BETWEEN (p_end + 1) AND (p_end + 7) AND extract(isodow FROM ip.next_roi_date::date) NOT IN (6,7)),
        'weekend_total', (SELECT coalesce(sum(round(ip.investment_amount * coalesce(ip.roi_percentage,15) / 100.0)),0) FROM investor_portfolios ip WHERE ip.status = 'active' AND ip.next_roi_date::date BETWEEN (p_end + 1) AND (p_end + 7) AND extract(isodow FROM ip.next_roi_date::date) IN (6,7)),
        'weekend_count', (SELECT count(*) FROM investor_portfolios ip WHERE ip.status = 'active' AND ip.next_roi_date::date BETWEEN (p_end + 1) AND (p_end + 7) AND extract(isodow FROM ip.next_roi_date::date) IN (6,7))
      )
    ),
    'mix', (
      SELECT jsonb_build_object(
        'by_mode', coalesce((
          SELECT jsonb_agg(jsonb_build_object('label', m.label, 'count', m.cnt, 'amount', m.amt) ORDER BY m.amt DESC)
          FROM (
            SELECT CASE WHEN roi_mode IN ('monthly_compounding','compound') THEN 'Compounding' ELSE 'Monthly payout' END AS label,
                   count(*) AS cnt, coalesce(sum(investment_amount),0) AS amt
            FROM investor_portfolios WHERE status = 'active'
            GROUP BY 1
          ) m
        ), '[]'::jsonb),
        'by_band', coalesce((
          SELECT jsonb_agg(jsonb_build_object('label', b.label, 'count', b.cnt, 'amount', b.amt) ORDER BY b.ord)
          FROM (
            SELECT CASE
