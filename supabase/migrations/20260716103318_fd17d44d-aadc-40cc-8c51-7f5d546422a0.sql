...
BEGIN
  ...
  -- EXEMPTION: Landlord float payouts are not the agent's personal cash-out.
  -- The float was already deducted from the agent's landlord-float bucket by
  -- landlord-payout-disburse (its own guards apply). This withdrawal_requests
  -- row only queues merchant fulfilment, so it must bypass the personal KYC
  -- daily cap that governs an agent's own withdrawable balance.
  ...
  IF (v_today_amount + NEW.amount) > v_effective_cap THEN
    IF v_is_new_user THEN
      ...
      NEW.status := 'rejected';
      NEW.rejection_reason := 'New account (< 7 days) daily cap UGX ' || v_new_user_cap::text
        || ' exceeded. Attempt flagged for fraud review.';
      RETURN NEW;
    END IF;
