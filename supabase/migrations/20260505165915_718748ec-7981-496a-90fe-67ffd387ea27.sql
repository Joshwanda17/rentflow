...
-- Replace the strict wallet view: only exclude system_balance_correction when it is admin_correction.
...
buckets AS (
  SELECT
    ledger.user_id,
    SUM(CASE
      WHEN COALESCE(ledger.category,'') <> ALL (ARRAY['agent_float_deposit','agent_float_used_for_rent','agent_float_settlement','agent_float_assignment','rent_float_funding','partner_funding'])
       AND COALESCE(ledger.category,'') NOT LIKE 'advance_%'
      THEN CASE WHEN ledger.direction='cash_in' THEN ledger.amount
                WHEN ledger.direction='cash_out' THEN -ledger.amount ELSE 0 END
      ELSE 0 END) AS withdrawable_raw,
    SUM(CASE
      WHEN COALESCE(ledger.category,'') = ANY (ARRAY['agent_float_deposit','agent_float_used_for_rent','agent_float_settlement','agent_float_assignment','rent_float_funding','partner_funding'])
      THEN CASE WHEN ledger.direction='cash_in' THEN ledger.amount
                WHEN ledger.direction='cash_out' THEN -ledger.amount ELSE 0 END
  ...
  FROM ledger GROUP BY ledger.user_id
),
holds AS (
...
-- Mirror the same rule in get_user_available_balance.
...
BEGIN
  ...
  WHERE user_id = p_user_id
    ...
    AND NOT (
      COALESCE(classification,'') = 'admin_correction'
