...
BEGIN
  ...
  SELECT phone, true INTO v_phone, v_signed_up FROM public.profiles WHERE id = p_referred_id;
  v_signed_up := COALESCE(v_signed_up, false);
  ...
  -- Sign-up alone qualifies the referrer's bonus
  v_qualified := v_signed_up;
...
FOR EACH ROW EXECUTE FUNCTION public.credit_signup_referral_bonus();