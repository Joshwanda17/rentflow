...
BEGIN
  ...
  SELECT
    ...
    count(*) FILTER (WHERE p.verified)::bigint AS verified,
    count(*) FILTER (WHERE p.last_active_at >= now() - interval '30 days')::bigint AS active30d,
    count(*) FILTER (WHERE p.created_at >= now() - interval '30 days')::bigint AS new30d
  FROM public.profiles p
  JOIN public.user_roles ur ON ur.user_id = p.id AND ur.role = _role;
END;
$$;

REVOKE ALL ON FUNCTION public.get_crm_directory(app_role, text, int, int) FROM public, anon;
REVOKE ALL ON FUNCTION public.get_crm_directory_totals(app_role) FROM public, anon;
GRANT EXECUTE ON FUNCTION public.get_crm_directory(app_role, text, int, int) TO authenticated;
