-- Mission Board: per-driver entity breakdown (agents, tenants, landlords, funders) behind each priority
create or replace function public.welile_mission_driver_entities(p_driver text, p_since timestamptz default null)
returns table (
  entity_type text,
  entity_id uuid,
  name text,
...
grant execute on function public.welile_mission_driver_entities(text, timestamptz) to authenticated, service_role;