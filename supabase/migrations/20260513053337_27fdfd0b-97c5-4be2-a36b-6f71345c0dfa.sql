...
BEGIN
  ...
  LOOP
    ...
    -- ===== ADVANCE-RECOVERY ROUTING GUARD (Wallet Routing v2) =====
    -- Repayment categories must never silently land in float/operational
    -- wallets. The user-side (wallet) leg requires an explicit recipient_type;
    -- the routing-compatibility check then enforces it can only be 'user'.
    v_category      := v_entry->>'category';
    v_scope         := COALESCE(v_entry->>'ledger_scope', 'wallet');
    v_recipient_type := NULLIF(v_entry->>'recipient_type', '');

    IF v_category IN ('agent_repayment','agent_advance_repayment','salary_advance_repayment','debt_recovery') THEN
      IF v_scope = 'wallet' THEN
