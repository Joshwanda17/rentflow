...
BEGIN
  ...
  -- Detect agent-managed using the actual profile columns. `is_managed_account`
  -- does not exist on public.profiles — use managed_by_agent / managing_agent_id,
  -- or fall back to "no email AND no phone" (proxy-style accounts).
  SELECT
