-- 1) Map rows for the two counter-legs used by Financial Ops physical cash deposits.
--    debit_when = 'cash_in' means a cash_out leg CREDITS the account.
INSERT INTO public.ledger_account_map (ledger_scope, category, wallet_bucket, account_code, debit_when, notes)
VALUES
  ('platform', 'cash_custody_payable',    NULL, 'L1', 'cash_in',
   'Custody obligation raised when Financial Ops receives physical cash for a wallet/float deposit'),
  ('platform', 'agent_float_cash_offset', NULL, 'A2', 'cash_in',
   'Offsets the wallet-scope float debit on a physical cash deposit: the cash is held by the company (A5), not by the agent')
...
-- 2) Extend the locked ledger category allowlist with the two new categories.
...
-- 3) Mark-as-Banked: step 4a (legacy A1 reclass) now only runs for deposits that
--    were approved BEFORE the approval flow started posting A5 directly.
...
BEGIN
  ...
  -- 4a) LEGACY ONLY: deposits approved before the approval flow posted A5 directly
  --     had their cash debited to A1. Reclass it into Cash in Transit exactly once.
