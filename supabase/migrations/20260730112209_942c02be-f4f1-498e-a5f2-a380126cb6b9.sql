CREATE OR REPLACE FUNCTION public.credit_agent_event_bonus(p_agent_id uuid, p_event_type text, p_tenant_id uuid DEFAULT NULL::uuid, p_source_id text DEFAULT NULL::text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  INSERT INTO commission_accrual_ledger (
    agent_id, event_type, tenant_id, source_id, amount, description, status, earned_at, created_at
  ) VALUES (
    p_agent_id, p_event_type, p_tenant_id, p_source_id, v_amount, v_description, 'pending', v_now, v_now
  )
