...
AS $function$
  ...
  linked_ids AS (
    ...
    WHERE a.uid IS NOT NULL AND rr.agent_id = a.uid AND rr.tenant_id IS NOT NULL
  ...
  WHERE p.id = p_tenant_id
    ...
    AND (
      ...
      OR EXISTS (
        ...
        WHERE rr.agent_id = auth.uid() AND rr.tenant_id = p.id
      )
...
GRANT EXECUTE ON FUNCTION public.get_agent_tenant_profile(uuid) TO authenticated;