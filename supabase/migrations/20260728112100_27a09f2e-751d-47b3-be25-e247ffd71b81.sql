...
WITH affected_users AS (
  ...
  FROM public.general_ledger gl
  WHERE gl.source_table = 'agent_advances'
    AND gl.category = 'agent_repayment'
    AND gl.direction = ANY (ARRAY['cash_out','debit'])
    AND gl.idempotency_key LIKE 'arrears_recover_%'
    AND EXISTS (
...
SELECT public.refresh_wallet_projection_for(user_id)
FROM affected_users;

SELECT public.refresh_wallet_projection_for('1409e6e0-89e7-46a6-89c8-feb3207b1df9'::uuid);