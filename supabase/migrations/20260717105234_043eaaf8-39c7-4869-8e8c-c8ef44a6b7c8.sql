...
BEGIN
  ...
  -- Proxy-withdrawal detection: the initiating agent is not the beneficiary.
  -- The funds are held in the AGENT's wallet pool (tagged with linked_party
  -- for legacy custody, or held in the managed-proxy float for v2), so the
  -- balance check must be against the agent, not the partner.
  v_is_proxy := (
