CREATE OR REPLACE FUNCTION public.get_agent_leaderboard_stats(p_period text DEFAULT 'monthly'::text)
 RETURNS jsonb
 LANGUAGE plpgsql
...
BEGIN
  IF p_period = 'daily' THEN
    v_trunc := 'day';
    v_series_start := date_trunc('day', now())   - interval '29 days';
    v_cur_start    := date_trunc('day', now());
    v_prev_start   := v_cur_start - interval '1 day';
  ELSIF p_period = 'weekly' THEN
    v_trunc := 'week';
    v_series_start := date_trunc('week', now())  - interval '11 weeks';
    v_cur_start    := date_trunc('week', now());
    v_prev_start   := v_cur_start - interval '1 week';
  ELSIF p_period = 'yearly' THEN
    v_trunc := 'year';
    v_series_start := date_trunc('year', now())  - interval '4 years';
    v_cur_start    := date_trunc('year', now());
    v_prev_start   := v_cur_start - interval '1 year';
  ELSE
    p_period := 'monthly';
    v_trunc := 'month';
    v_series_start := date_trunc('month', now()) - interval '11 months';
    v_cur_start    := date_trunc('month', now());
    v_prev_start   := v_cur_start - interval '1 month';
  END IF;

  -- Canonical behaviour-based agents (NOT user_roles role='agent', which is
  -- auto-assigned to nearly everyone). Anchored to profile join date so we can
  -- bucket "new agents" over time.
  CREATE TEMP TABLE _qa ON COMMIT DROP AS
  SELECT q.agent_id, p.created_at
  FROM public.agent_ops_qualifying_agent_ids() q
  JOIN public.profiles p ON p.id = q.agent_id;

  SELECT count(*) INTO v_new_agents
  FROM _qa WHERE created_at >= v_cur_start;

  SELECT count(*) INTO v_new_subagents
  ...
  SELECT jsonb_build_object(
    'total_agents',       (SELECT count(*) FROM _qa),
  ...
  WITH buckets AS (
    SELECT generate_series(v_series_start, date_trunc(v_trunc, now()), ('1 ' || v_trunc)::interval) AS b
  ),
  ag AS (
  ...
  -- top recruiters within the CURRENT selected period
  ...
  RETURN jsonb_build_object(
    'period', p_period,
    'window_start', v_cur_start,
    'series_start', v_series_start,
    'totals', v_totals,
