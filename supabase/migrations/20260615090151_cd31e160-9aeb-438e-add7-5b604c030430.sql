DROP FUNCTION IF EXISTS public.ceo_angel_pool_shareholder_action(uuid, text, text, integer);

CREATE OR REPLACE FUNCTION public.ceo_angel_pool_shareholder_action(
  p_investor_id uuid,
  p_action text,
  p_reason text,
  p_new_shares numeric DEFAULT NULL
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  ELSIF p_action = 'edit' THEN
    IF p_new_shares IS NULL OR p_new_shares < 0 THEN
      RAISE EXCEPTION 'p_new_shares must be a non-negative decimal number for edit';
    END IF;
    ...
    SET shares = p_new_shares,
        amount = round(p_new_shares * v_price)::bigint,
        pool_ownership_percent = (p_new_shares / v_total_shares) * 100,
        company_ownership_percent = (p_new_shares / v_total_shares) * v_pool_pct
    WHERE investor_id = p_investor_id
      AND status = 'confirmed';
    GET DIAGNOSTICS v_affected = ROW_COUNT;
  END IF;
...
REVOKE ALL ON FUNCTION public.ceo_angel_pool_shareholder_action(uuid, text, text, numeric) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.ceo_angel_pool_shareholder_action(uuid, text, text, numeric) TO authenticated;