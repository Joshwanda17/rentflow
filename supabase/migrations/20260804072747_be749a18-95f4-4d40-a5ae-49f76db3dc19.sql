-- =====================================================================
-- Agent Advance Limit engine: 70% reduction + re-prioritised drivers
--
-- Base:            30,000  ->  20,000
-- Ceiling:     30,000,000  ->  9,000,000
-- Driver order (largest contribution first):
--   1. active sub-agents   cap 3,000,000
--   2. rent collected      cap 2,400,000  (6% of collections)
--   3. rent requests       cap 1,500,000
--   4. promissory notes    cap   600,000
-- Houses-listed and tenants-placed bonuses are retired for agents.
-- =====================================================================

-- 1. Ceiling on the derived total (generated column must be re-created).
-- The starter-limit RLS policy references total_limit, so it is rebuilt with
-- the new UGX 20,000 starter figure.
DROP POLICY IF EXISTS "Users can insert own starter credit limit" ON public.credit_access_limits;
ALTER TABLE public.credit_access_limits DROP COLUMN total_limit;
ALTER TABLE public.credit_access_limits
  ADD COLUMN total_limit numeric GENERATED ALWAYS AS (
    LEAST(
...
WITH CHECK (
  auth.uid() = user_id
  AND COALESCE(base_limit, 20000::numeric) = 20000::numeric
  AND COALESCE(bonus_from_ratings, 0::numeric) = 0::numeric
  AND COALESCE(bonus_from_receipts, 0::numeric) = 0::numeric
  AND COALESCE(bonus_from_rent_history, 0::numeric) = 0::numeric
  AND COALESCE(bonus_from_landlord_rent, 0::numeric) = 0::numeric
  AND COALESCE(bonus_from_houses_listed, 0::numeric) = 0::numeric
  AND COALESCE(bonus_from_partners_onboarded, 0::numeric) = 0::numeric
  AND COALESCE(bonus_from_agent_allocations, 0::numeric) = 0::numeric
  AND COALESCE(bonus_from_subagents, 0::numeric) = 0::numeric
  AND COALESCE(total_limit, 20000::numeric) = 20000::numeric
);

-- 2. The stored-limit engine.
...
-- 3. The Agent Ops display engine must mirror the stored engine exactly.
...
RETURNS TABLE (
  ...
  verified boolean, territory text,
  direct_subagents bigint, registered_subagents bigint, active_subagents bigint,
  rent_collected numeric, collections_count bigint,
  houses_listed bigint, rent_requests bigint, tenants_registered bigint,
  promissory_notes bigint,
  base_limit numeric, subagents_bonus numeric, tenants_bonus numeric,
  collections_bonus numeric, promissory_bonus numeric, houses_bonus numeric,
  requests_bonus numeric, total_limit numeric, stored_total_limit numeric,
...
BEGIN
  ...
  computed AS (
    SELECT m.*,
      20000::numeric AS c_base,
      LEAST((m.active_subs * 30000) + (m.registered_subs * 9000), 3000000)::numeric AS c_subs,
      LEAST(m.rent_collected * 0.06, 2400000)::numeric AS c_coll,
      LEAST(m.rent_requests * 9000, 1500000)::numeric AS c_req,
      LEAST(m.promissory_notes * 9000, 600000)::numeric AS c_promis,
      0::numeric AS c_tenants,
      0::numeric AS c_house
    FROM metrics m
  ),
  finalized AS (
    SELECT cp.*, LEAST(cp.c_base + cp.c_subs + cp.c_coll + cp.c_req + cp.c_promis, 9000000)::numeric AS c_total
    FROM computed cp
  ),
...
REVOKE ALL ON FUNCTION public.get_agent_advance_limits(text, integer, integer) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_agent_advance_limits(text, integer, integer) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.recalculate_credit_limit(uuid) TO authenticated, service_role;
