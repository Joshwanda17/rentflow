...
--
-- The 4-arg overload of recompute_agent_earned_vouch was inserting into a
-- non-existent `updated_at` column on welile_trust_score_cache, crashing the
-- agent tenant float allocation "Confirm Payment" step. The actual column
-- name is `last_calculated_at` (see schema and the 1-arg overload, which is
-- already correct).
