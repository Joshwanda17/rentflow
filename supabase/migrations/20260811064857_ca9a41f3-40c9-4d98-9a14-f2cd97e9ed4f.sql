...
BEGIN
  ...
  VALUES (
    ...
    COALESCE(NULLIF(p_note, ''), 'Cash location updated to ' || v_loc || ' by finance staff'),
    jsonb_build_object('cash_location', v_loc, 'previous', v_prev)
  );
...
GRANT EXECUTE ON FUNCTION public.fin_ops_set_cash_location(uuid, text, text) TO authenticated;