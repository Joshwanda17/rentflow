...
AS $function$
  ...
  WHERE auth.uid() IS NOT NULL
    ...
    -- Compounding portfolios never receive a cash payout: their returns roll
    -- back into the portfolio. They must not appear in a proxy agent's payout
    -- queue (Kabahuma Lillian was seeing compounded partners as payable).
    AND COALESCE(ip.roi_mode, 'monthly_payout') NOT IN ('monthly_compounding', 'compound')
