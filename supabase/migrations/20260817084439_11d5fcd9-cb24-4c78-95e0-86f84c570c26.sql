...
BEGIN
  ...
  -- The last ledger-posted, evidence-carrying reconciliation is an accounting
  -- ANCHOR: from that moment the desk float is the figure finance evidenced and
  -- posted, not the all-time raw sum (which still carries pre-anchor phantom
  -- legs from the 14-Aug duplicate-reversal incident). Measure the books as
  -- "anchored figure + everything that moved after it", which is exactly what
  -- the wallet projection and set_merchant_desk_float_to measure.
  baseline AS (
    SELECT DISTINCT ON (r.desk_id)
           r.desk_id,
           r.created_at AS anchored_at,
           CASE WHEN r.adjustment_type = 'evidenced_writedown'
                THEN r.stored_float_at_post - r.amount
                ELSE r.stored_float_at_post + r.amount END AS anchored_float
    FROM public.merchant_float_reconciliations r
    WHERE r.ledger_effect = 'ledger_posted'
      AND r.stored_float_at_post IS NOT NULL
    ORDER BY r.desk_id, r.created_at DESC
  ),
  -- "The books" measured EXACTLY as the wallet projection measures it.
  raw_float_net AS (
    SELECT d.id AS desk_id,
           COALESCE(b.anchored_float, 0)
           + COALESCE(SUM(CASE WHEN g.direction IN ('cash_in','credit') THEN g.amount ELSE -g.amount END), 0) AS net
    FROM desks d
    LEFT JOIN baseline b ON b.desk_id = d.id
    LEFT JOIN public.general_ledger g
      ON g.user_id = d.agent_id
     AND g.ledger_scope = 'wallet'
     AND g.wallet_bucket = 'float'
     AND (b.anchored_at IS NULL OR g.transaction_date > b.anchored_at)
     AND (
    ...
    GROUP BY d.id, b.anchored_float
  ),
