CREATE OR REPLACE FUNCTION public.get_withdrawal_history(p_search text DEFAULT NULL::text, p_limit integer DEFAULT 100, p_offset integer DEFAULT 0)
 RETURNS TABLE(withdrawal_id uuid, user_id uuid, user_name text, user_phone text, amount numeric, status text, payout_method text, transaction_id text, mobile_money_number text, mobile_money_provider text, bank_name text, bank_account_number text, created_at timestamp with time zone, processed_at timestamp with time zone, balance_before numeric, balance_after numeric, total_count bigint)
 LANGUAGE plpgsql
...
BEGIN
  ...
  WITH filtered AS (
    ...
    WHERE wr.landlord_payout_id IS NULL
      AND COALESCE(wr.reason, '') NOT ILIKE 'Landlord float payout%'
      AND (
