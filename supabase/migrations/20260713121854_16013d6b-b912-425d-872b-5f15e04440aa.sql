CREATE OR REPLACE FUNCTION public.is_platform_user_admin(_user_id uuid DEFAULT auth.uid())
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT COALESCE(
    ...
    OR public.has_role(_user_id, 'coo'::public.app_role)
    OR public.has_role(_user_id, 'cfo'::public.app_role)
    OR public.has_role(_user_id, 'cto'::public.app_role)
    OR public.has_role(_user_id, 'cmo'::public.app_role)
    OR public.has_role(_user_id, 'crm'::public.app_role)
    OR public.has_role(_user_id, 'operations'::public.app_role)
...
CREATE OR REPLACE FUNCTION public.get_platform_users_page(
  _search text DEFAULT '',
  _role_filter text DEFAULT 'all',
  _verification_filter text DEFAULT 'all',
  _stat_filter text DEFAULT 'all',
...
BEGIN
  ...
  WITH filtered AS (
    ...
    WHERE
      ...
      AND (
        v_role NOT IN ('tenant', 'agent', 'supporter', 'landlord', 'manager')
        OR EXISTS (
          SELECT 1
...
REVOKE ALL ON FUNCTION public.is_platform_user_admin(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.get_platform_user_counts() FROM PUBLIC;
REVOKE ALL ON FUNCTION public.get_platform_users_page(text, text, text, text, text, integer, integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.is_platform_user_admin(uuid) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.get_platform_user_counts() TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.get_platform_users_page(text, text, text, text, text, integer, integer) TO authenticated, service_role;
...
CREATE INDEX IF NOT EXISTS idx_user_roles_role_enabled_user_id ON public.user_roles (role, enabled, user_id);
