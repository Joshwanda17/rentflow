...
BEGIN
  BEGIN
    ...
    INSERT INTO agent_subagents(parent_agent_id,sub_agent_id,status,source)
      VALUES (parent,a,'verified','test'),(parent,b,'verified','test');
    INSERT INTO user_roles(user_id,role) VALUES (ops,'agent_ops') ON CONFLICT DO NOTHING;
    INSERT INTO landlords(name,phone,registered_by,property_address,verified,verified_at,verification_status)
      VALUES ('SC Smoke LL','0700000111',a,'Sandbox Road',true,now(),'verified') RETURNING id INTO ll;
    INSERT INTO rent_requests(tenant_id,landlord_id,agent_id,assigned_agent_id,rent_amount,status,tenant_photo_url)
      VALUES (ten,ll,parent,a,200000,'funded','https://example.com/p.jpg') RETURNING id INTO rr;
    ...
    EXCEPTION WHEN others THEN log := log || ('2 duplicate pending blocked=PASS'); END;
    ...
    EXCEPTION WHEN others THEN log := log || ('3 unlink blocked on assigned active plan=PASS'); END;
    ...
    EXCEPTION WHEN others THEN log := log || ('5 suspended recipient blocked=PASS'); END;
    ...
    EXCEPTION WHEN others THEN log := log || ('7 restore with no parent block errors=PASS'); END;
    ...
    EXCEPTION WHEN others THEN log := log || ('9 approve on inactive plan refused=PASS'); END;
    ...
    log := log || ('10a link removed on unlink='||CASE WHEN msg='0' THEN 'PASS' ELSE 'FAIL remaining='||msg END);
    SELECT count(*)::text INTO msg FROM agent_subagent_link_archive WHERE parent_agent_id=parent AND sub_agent_id=a;
    log := log || ('10b link archived='||CASE WHEN msg='1' THEN 'PASS' ELSE 'FAIL archived='||msg END);
