...
DECLARE
  ...
  v_known_total numeric;     -- annual receivable from houses WITH a rent value
...
BEGIN
  ...
  SELECT
    COALESCE(SUM(rent) FILTER (WHERE rent > 0), 0) * 12,
  ...
  SELECT
    ...
    COALESCE((SELECT SUM(COALESCE(l.monthly_rent, l.desired_rent_from_welile, 0)) * 12
              FROM public.landlords l
              WHERE l.registered_by IS NOT NULL
                AND l.tenant_id IS NULL
                AND NOT EXISTS (SELECT 1 FROM public.house_listings h WHERE h.landlord_id = l.id)
    ...
    v_missing_count,
    v_avg_monthly,
    (v_known_total + (v_missing_count * v_avg_monthly * 12))::numeric,
    v_earliest;
END;
$function$;
