-- Typo-tolerant / fuzzy landlord search using pg_trgm trigram similarity.
-- Optimised for scale via GIN trigram indexes on name + phone.
...
as $$
  ...
  where
    q.term = ''
    -- exact substring matches (name or phone)
    or l.name ilike '%' || q.term || '%'
    or l.phone ilike '%' || q.term || '%'
    -- phone digit-only match (ignore spaces/dashes/+)
    or (length(q.digits) >= 3 and l.phone ilike '%' || q.digits || '%')
    -- fuzzy / typo-tolerant matches on name
