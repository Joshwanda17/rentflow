...
-- Stores periodic Google Search Console snapshots (sitemap indexed count +
-- URL inspection verdict) and a single-row settings record that controls
-- alert email + on/off. Written by the `seo-index-monitor` edge function
-- (service role); read by ops staff in the CTO dashboard.
...
CREATE TABLE IF NOT EXISTS public.seo_index_monitor_snapshots (
  ...
  alert_type text,          -- null | 'first_indexation' | 'errors' | 'errors_cleared'
