ALTER TABLE public.cashout_agents
  ADD COLUMN IF NOT EXISTS float_phone text,
  ADD COLUMN IF NOT EXISTS personal_phone text,
  ADD COLUMN IF NOT EXISTS payout_numbers_set_at timestamptz,
  ADD COLUMN IF NOT EXISTS payout_numbers_set_by uuid;

ALTER TABLE public.cashout_agents
  DROP CONSTRAINT IF EXISTS cashout_agents_float_phone_format;
ALTER TABLE public.cashout_agents
  ADD CONSTRAINT cashout_agents_float_phone_format
  CHECK (float_phone IS NULL OR float_phone ~ '^2567[0-9]{8}$');

ALTER TABLE public.cashout_agents
  DROP CONSTRAINT IF EXISTS cashout_agents_personal_phone_format;
ALTER TABLE public.cashout_agents
  ADD CONSTRAINT cashout_agents_personal_phone_format
  CHECK (personal_phone IS NULL OR personal_phone ~ '^2567[0-9]{8}$');

ALTER TABLE public.cashout_agents
  DROP CONSTRAINT IF EXISTS cashout_agents_payout_phones_distinct;
ALTER TABLE public.cashout_agents
  ADD CONSTRAINT cashout_agents_payout_phones_distinct
  CHECK (float_phone IS NULL OR personal_phone IS NULL OR float_phone <> personal_phone);
...
BEGIN
  ...
  v_float := public.normalize_ug_phone_strict(p_float_phone);
  v_personal := public.normalize_ug_phone_strict(p_personal_phone);

  IF v_float IS NULL OR v_personal IS NULL THEN
