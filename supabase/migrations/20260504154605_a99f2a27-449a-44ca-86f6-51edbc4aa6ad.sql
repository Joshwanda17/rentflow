  ...
  LOOP
    ...
    ELSIF v_entry->>'direction' = 'cash_out' THEN
      ...
      IF NOT skip_balance_check
         ...
         AND (v_entry->>'user_id') IS NOT NULL THEN

        -- Aligned with public.get_user_available_balance:
        --  * honor the fresh-start anchor (skip pre-anchor legacy drag)
        --  * exclude float-bucket categories (company liability, not the
        --    user's own withdrawable funds)
        --  * cap by the cached withdrawable_balance bucket
        ...
        -- Strict cap: cannot exceed cached withdrawable AND cannot exceed
        -- the post-anchor, float-excluded ledger net.
        v_user_balance := GREATEST(0,
          LEAST(COALESCE(v_cached_withdrawable, 0), GREATEST(0, v_user_balance))
        );
