...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'errors', jsonb_build_object(
      ...
      'daily_trend', COALESCE((
        SELECT jsonb_agg(x ORDER BY (x->>'d')) FROM (
          SELECT jsonb_build_object('d', (created_at AT TIME ZONE 'Africa/Kampala')::date, 'n', count(*)) AS x
    ...
    'security', jsonb_build_object(
      ...
      'rls_tables', (SELECT count(*) FROM pg_tables t JOIN pg_class c ON c.relname = t.tablename AND c.relnamespace = 'public'::regnamespace WHERE t.schemaname='public' AND c.relrowsecurity),
    ...
    'infra', jsonb_build_object(
      ...
      'uptime_hours', (SELECT round(EXTRACT(epoch FROM (now() - pg_postmaster_start_time()))/3600.0, 1)),
...
REVOKE ALL ON FUNCTION public.get_cto_daily_report(date) FROM public;
GRANT EXECUTE ON FUNCTION public.get_cto_daily_report(date) TO authenticated, service_role;