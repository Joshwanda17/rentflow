CREATE OR REPLACE VIEW public.agent_advance_requests_privileged AS
 SELECT aar.id,
    ...
    aar.request_kind,
    aar.parent_advance_id,
    aar.extend_days,
    aar.repayment_frequency
   FROM agent_advance_requests aar
     LEFT JOIN profiles p ON p.id = aar.agent_id
  WHERE has_role(auth.uid(), 'super_admin'::app_role) OR has_role(auth.uid(), 'manager'::app_role) OR has_role(auth.uid(), 'cfo'::app_role) OR has_role(auth.uid(), 'coo'::app_role) OR (EXISTS ( SELECT 1
           FROM staff_permissions sp
          WHERE sp.user_id = auth.uid() AND (sp.permitted_dashboard = ANY (ARRAY['agent-ops'::text, 'tenant-ops'::text, 'landlord-ops'::text, 'financial-ops'::text, 'company-ops'::text]))));