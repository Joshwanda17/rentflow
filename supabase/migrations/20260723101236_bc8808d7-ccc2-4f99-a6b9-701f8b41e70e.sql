...
BEGIN
  ...
  -- CFO-allocated landlord float pool is the sole capacity signal here.
  -- Lock the row so concurrent allocations can't double-spend the pool.
  ...
  IF v_float_balance < p_amount THEN
    RETURN jsonb_build_object(
      'success', false,
      'error_code', 'INSUFFICIENT_LANDLORD_FLOAT',
      'error', format(
        'Insufficient landlord float. Available: %s, Requested: %s. Ask the CFO to top up your landlord float.',
        v_float_balance, p_amount
      ),
      'landlord_float', v_float_balance
    );
  END IF;
  ...
  -- Landlord payout is drawn from the platform's landlord-float pool
  -- (tracked separately in agent_landlord_float), NOT the agent's wallet
  -- float bucket. Ledger legs:
  --   1. platform cash_out   → agent_landlord_payout (pool spent)
  ...
  v_legs := jsonb_build_array(
    jsonb_build_object(
      ...
      'description', 'Landlord payout from CFO-allocated landlord float pool',
    ...
    jsonb_build_object(
      ...
      'ledger_scope', 'wallet',
      'classification', 'production',
      'description', '10% commission on landlord float allocation',
      'recipient_type', 'user',
      'source_table', 'agent_collections',
      'source_id', p_rent_request_id
    ),
  ...
  IF v_parent_agent_id IS NOT NULL AND v_parent_override > 0 THEN
    v_legs := v_legs || jsonb_build_array(
      jsonb_build_object(
        ...
        'ledger_scope', 'wallet',
        'classification', 'production',
        'description', '2% recruiter override on sub-agent landlord allocation',
        'recipient_type', 'user',
        'source_table', 'agent_collections',
        'source_id', p_rent_request_id
      )
  ...
  -- Draw down the CFO landlord-float pool for this agent.
  ...
  RETURN jsonb_build_object(
    ...
    'landlord_float_before', v_float_balance,
    'landlord_float_after', GREATEST(0, v_float_balance - p_amount),
