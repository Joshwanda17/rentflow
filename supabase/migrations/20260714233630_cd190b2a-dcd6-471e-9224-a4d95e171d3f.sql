...
CREATE OR REPLACE FUNCTION public.get_agent_advance_limits(_search text DEFAULT NULL::text, _limit integer DEFAULT 100, _offset integer DEFAULT 0)
 RETURNS TABLE(agent_id uuid, full_name text, phone text, email text, avatar_url text, verified boolean, territory text, direct_subagents bigint, registered_subagents bigint, active_subagents bigint, rent_collected numeric, collections_count bigint, houses_listed bigint, rent_requests bigint, base_limit numeric, subagents_bonus numeric, collections_bonus numeric, houses_bonus numeric, requests_bonus numeric, total_limit numeric, stored_total_limit numeric, total_matched bigint)
 LANGUAGE plpgsql
...
BEGIN
  ...
  computed AS (
    SELECT m.*,
      ...
      ((m.registered_subs * 30000) + (m.active_subs * 50000))::numeric AS c_subs,
