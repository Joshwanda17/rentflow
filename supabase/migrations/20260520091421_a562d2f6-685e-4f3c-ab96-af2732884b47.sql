...
begin
  ...
  loop
    ...
    -- ═══ DEPOSIT PURPOSE INFERENCE (SMS → purpose) ═══
    -- Only auto-tag when the matched user actually wears the agent hat;
    -- otherwise leave purpose NULL so the user is forced to confirm.
    v_is_agent := exists (
