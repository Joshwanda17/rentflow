CREATE OR REPLACE FUNCTION public.get_service_center_listing_queue(p_manager_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
...
BEGIN
  ...
  FROM (
    SELECT jsonb_build_object(
      ...
      'landlord_name', COALESCE(ll.name, lp.full_name),
      'landlord_phone', COALESCE(ll.phone, lp.phone),
    ...
    LEFT JOIN public.landlords ll ON ll.id = h.landlord_id
