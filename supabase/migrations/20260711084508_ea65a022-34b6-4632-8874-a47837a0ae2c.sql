...
BEGIN
  ...
  FOR rec IN
    ...
    WHERE validation_result = 'mismatch' AND approver_id IS NOT NULL
  ...
  LOOP
    ...
    SELECT coalesce(email, full_name) INTO v_label FROM public.profiles WHERE id = rec.approver_id;
    ...
    IF v_severity IN ('high', 'critical') THEN
      ...
      VALUES ('sms_verification_failure_alert_raised', 'approver', rec.approver_id, jsonb_build_object(
  ...
  FOR rec IN
    ...
    FROM public.payout_claim_sms_audit_log
    WHERE validation_result = 'mismatch' AND withdrawal_request_id IS NOT NULL
      AND created_at >= v_window_start AND created_at < v_window_end
  ...
  LOOP
    ...
    IF v_severity IN ('high', 'critical') THEN
      INSERT INTO public.system_events (event_type, related_entity_type, related_entity_id, metadata)
      VALUES ('sms_verification_failure_alert_raised', 'withdrawal', rec.withdrawal_request_id, jsonb_build_object(
