...
CREATE OR REPLACE FUNCTION public.get_platform_user_counts()
 RETURNS TABLE(total_users bigint, verified_users bigint, inactive_users bigint, tenant_count bigint, agent_count bigint, supporter_count bigint, landlord_count bigint, manager_count bigint, super_admin_count bigint, employee_count bigint)
 LANGUAGE plpgsql
...
BEGIN
  ...
  SELECT
    ...
    (SELECT count(*) FROM public.user_roles WHERE role = 'employee'::public.app_role AND enabled IS TRUE)::bigint AS employee_count;
END;
$function$;

GRANT EXECUTE ON FUNCTION public.get_platform_user_counts() TO authenticated;
...
BEGIN
  ...
  WITH filtered AS (
    ...
    WHERE
      ...
      AND (
        v_role NOT IN ('tenant', 'agent', 'supporter', 'landlord', 'manager', 'super_admin', 'employee')
        OR EXISTS (
          SELECT 1
