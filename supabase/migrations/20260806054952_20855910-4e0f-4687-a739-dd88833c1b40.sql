...
BEGIN
  ...
  land_union AS (
    ...
    FROM (
      SELECT l.registered_by AS agent_id, l.id AS landlord_id, 'registered'::text AS link_source,
             l.name, l.phone, l.district, l.region, l.property_address, l.village, l.county,
             l.created_at,
      ...
      SELECT a.agent_id, l2.id AS landlord_id, 'assigned'::text AS link_source,
             l2.name, l2.phone, l2.district, l2.region, l2.property_address, l2.village, l2.county,
             l2.created_at,
  ...
  land_state AS (
    SELECT u.*,
           NULLIF(btrim(COALESCE(NULLIF(btrim(u.district), ''), hh.district, '')), '') AS district_resolved,
           NULLIF(btrim(COALESCE(NULLIF(btrim(u.region), ''), hh.region, '')), '') AS region_resolved,
           NULLIF(btrim(COALESCE(NULLIF(btrim(u.property_address), ''), hh.address, '')), '') AS address_resolved,
           CASE
    ...
    LEFT JOIN LATERAL (
      SELECT h.address, h.district, h.region
      FROM public.house_listings h
      WHERE h.landlord_id = u.landlord_id
      ORDER BY h.updated_at DESC NULLS LAST
      LIMIT 1
    ) hh ON true
  ),
    ...
    SELECT s.agent_id,
           ...
           jsonb_agg(
             jsonb_build_object(
               ...
               'name', s.name,
               'phone', s.phone,
               'district', s.district_resolved,
               'region', s.region_resolved,
               'address', s.address_resolved,
               'village', s.village,
               'county', s.county,
               'state', s.state,
  ...
  SELECT jsonb_build_object(
    'sub_agents', COALESCE(jsonb_agg(
      jsonb_build_object(
        ...
        'commission_total', GREATEST(COALESCE(e.commission_total,0), COALESCE(ac.commission_total,0)),
        ...
        'landlords_registered', COALESCE(l.landlords_registered,0),
        'landlords_verified', COALESCE(l.landlords_verified,0),
        'landlords_pending', COALESCE(l.landlords_pending,0),
        'landlords_rejected', COALESCE(l.landlords_rejected,0),
        'landlord_list', COALESCE(l.landlord_list,'[]'::jsonb),
        'houses_listed', COALESCE(h.houses_listed,0),
        'houses_verified', COALESCE(h.houses_verified,0),
        'houses_pending', COALESCE(h.houses_pending,0),
        'houses_rejected', COALESCE(h.houses_rejected,0),
        'house_list', COALESCE(h.house_list,'[]'::jsonb),
        'pending_transfers', COALESCE(tf.pending_transfers,0),
        'suspension', CASE WHEN b.agent_id IS NULL THEN NULL ELSE jsonb_build_object(
          'blocked_until', b.blocked_until,
          'reason', b.reason,
          'freeze_scope', b.freeze_scope
        ) END
  ...
  LEFT JOIN lands l ON l.agent_id = k.sub_agent_id
  LEFT JOIN houses h ON h.agent_id = k.sub_agent_id
  LEFT JOIN blocks b ON b.agent_id = k.sub_agent_id
  LEFT JOIN pending_tf tf ON tf.agent_id = k.sub_agent_id;

  RETURN COALESCE(v_result, jsonb_build_object('sub_agents','[]'::jsonb));
END;
$function$;