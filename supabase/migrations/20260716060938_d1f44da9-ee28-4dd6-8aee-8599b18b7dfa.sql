...
BEGIN
  ...
  IF (v_today_amount + NEW.amount) > v_effective_cap THEN
    IF v_is_new_user THEN
      -- Persist as rejected+flagged so admins see the fraud attempt.
      -- Flag & risk event insert BEFORE the row so they survive if downstream fails.
      ...
      -- Auto-reject the withdrawal row so it persists as an audit record
      NEW.status := 'rejected';
