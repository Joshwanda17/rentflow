CREATE OR REPLACE FUNCTION public.get_cashflow_forecast_series(
  p_start timestamptz,
  p_end timestamptz,
...
BEGIN
  ...
  -- 1. Forecast: projected partner Returns from active portfolios
  WITH p AS (
    SELECT ip.id, ip.investment_amount, ip.roi_percentage, ip.next_roi_date,
           ip.maturity_date, ip.roi_mode, ip.auto_reinvest
    ...
    SELECT p.id,
           (p.next_roi_date::date + (n || ' month')::interval)::date AS due_date,
           ROUND(p.investment_amount * p.roi_percentage / 100.0) AS amount,
           p.maturity_date
    FROM p CROSS JOIN generate_series(0, 36) AS n
  ), f AS (
    ...
    WHERE due_date >= (p_start AT TIME ZONE v_tz)::date
      ...
      AND (maturity_date IS NULL OR due_date <= maturity_date::date)
  )
  SELECT jsonb_build_array(jsonb_build_object(
           'key', 'roi_forecast',
           'label', 'Forecasted Returns (partner portfolios)',
           'kind', 'forecast',
...
GRANT EXECUTE ON FUNCTION public.get_cashflow_forecast_series(timestamptz, timestamptz, text) TO authenticated, service_role;