...
BEGIN
  ...
  RETURN QUERY
  -- Collapse to the single LATEST verification per deposit so a regenerated
  -- code always supersedes the stale one (no duplicate stacked rows).
  WITH latest AS (
  ...
  SELECT
    ...
    l.amount,
    CASE
      WHEN l.status = 'verified' THEN NULL
      ELSE l.code_plain
    END AS code,
    l.status,
    l.attempts,
