...
BEGIN
  ...
  -- Commission earnings are agent income, NOT custody of partner principal/ROI.
  -- These legs legitimately credit an agent wallet (including a recruiter's 2%
  -- sub-agent override) and may tag the counterparty agent — who can also hold
  -- a supporter role — so they must never be treated as proxy custody.
  IF NEW.category IN ('agent_commission_earned', 'agent_commission_payable') THEN
    RETURN NEW;
  END IF;
