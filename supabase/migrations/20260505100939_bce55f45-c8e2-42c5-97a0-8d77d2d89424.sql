...
BEGIN
  ...
  -- Idempotency is naturally enforced by the cached-vs-strict-net filter:
  -- a user already aligned will not appear in this loop. Do NOT skip on
  -- log-row presence — the older `reconcile_negative_wallets` only brings
  -- strict ledger to zero, not to cached, so those users still need the
  -- top-up posted here.
  FOR v_rec IN
    WITH net AS (
      SELECT user_id,
