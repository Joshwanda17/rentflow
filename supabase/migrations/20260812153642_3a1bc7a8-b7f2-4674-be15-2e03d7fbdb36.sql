...
BEGIN
  ...
  IF NOT v_completed THEN
    -- Money has not (yet) left. Never assert a funding source or receivable.
    ...
    VALUES (p_withdrawal_id, v_agent, v_amount, v_telecom, v_float_principal,
            ...
            'unknown', p_via,
            'Payout not completed (status=' || COALESCE(w.status,'null') || '); funding source undecided.')
    ...
    RETURN jsonb_build_object(
      ...
      'funding_source', 'unknown', 'status', w.status,
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      v_done := v_done + 1;
      IF v_res->>'funding_source' = 'own_cash' THEN v_own := v_own + 1; END IF;
      IF v_res->>'funding_source' = 'mixed' THEN v_mixed := v_mixed + 1; END IF;
    EXCEPTION WHEN OTHERS THEN
      NULL;
    END;
  END LOOP;

  RETURN jsonb_build_object('ok', true, 'classified', v_done,
                            'own_cash', v_own, 'mixed', v_mixed);
END;
$$;
...
SELECT public.reconcile_merchant_payout_funding(200000, 4000) AS batch_1;