...
BEGIN
  ...
  FROM business_advances WHERE status IN ('active','disbursed','defaulted') AND created_at <= p_as_at;
  ...
  -- ── LANDLORD PRODUCTS & SERVICES (Welile Homes 10%) ─────────────────────
  ...
  -- ── R&D / VENTURES (ledger-tagged, zero until posted) ───────────────────
