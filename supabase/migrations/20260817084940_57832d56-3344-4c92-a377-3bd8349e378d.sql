...
DECLARE
  r record;
  v_author uuid := '29a0cfa8-1eaf-453c-874c-0fc72fa4f74b'; -- Angwen Sarah (CFO)
  v_recon_id uuid;
BEGIN
  ...
  FOR r IN
    ...
    nets AS (
      ...
      GROUP BY d.desk_id, d.agent_id, d.cache
    )
    SELECT desk_id, agent_id, cache, net, round(cache - net) AS delta
    FROM nets
    WHERE cache > 0
      AND round(cache - net) > 0
      AND agent_id <> v_author
  LOOP
    ...
    VALUES
      (r.desk_id, r.agent_id, 'opening_balance', r.delta,
       'Opening balance reconciliation: recognise the float the desk actually holds as a ledger-backed starting balance',
       format('Starting balance confirmed by finance on 17/08/2026 as UGX %s. Books showed UGX %s before this entry; this double-entry post also absorbs the phantom duplicate-reversal debits from the 14-Aug incident so the desk float is fully ledger supported.', r.cache, r.net),
       v_author, 'ledger_posted')
    ...
    PERFORM public.create_ledger_transaction(
      entries := jsonb_build_array(
        jsonb_build_object(
          ...
          'description', format('Merchant desk opening balance recognised: UGX %s (books showed %s)', r.cache, r.net),
          'currency', 'UGX',
        ...
        jsonb_build_object(
          ...
          'source_table', 'merchant_float_reconciliations',
          'source_id', v_recon_id,
          'description', 'Platform: merchant desk opening balance recognised',
          'currency', 'UGX',
          'transaction_date', now()
        )
      ),
      idempotency_key := 'merchant_opening_balance:' || v_recon_id::text,
...
BEGIN
  ...
  -- "The books" measured EXACTLY as the wallet projection measures it: the full
  -- ledger history, now that every desk opening balance is a real double-entry post.
  raw_float_net AS (
    SELECT d.id AS desk_id,
           COALESCE(SUM(CASE WHEN g.direction IN ('cash_in','credit') THEN g.amount ELSE -g.amount END), 0) AS net
    FROM desks d
