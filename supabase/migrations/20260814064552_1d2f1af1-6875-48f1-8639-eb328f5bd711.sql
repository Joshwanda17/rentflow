...
BEGIN
  ...
  WITH agents AS (
    ...
    FROM profiles p
    WHERE p.id IN (SELECT agent_id FROM public.agent_ops_strict_agent_ids())
  ),
  rent_agg AS (
    SELECT
  ...
  adv_outstanding AS (
    SELECT COALESCE(sum(av.outstanding_balance),0) AS outstanding, count(*) AS active_count
    FROM agent_advances av WHERE av.status = 'active'
  ),
  adv_rows AS (
    SELECT av.id,
    ...
    WHERE av.status = 'active' OR (av.issued_at >= v_start AND av.issued_at < v_end)
  ),
  sc AS (
    SELECT
