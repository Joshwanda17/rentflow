CREATE OR REPLACE FUNCTION public.renew_rent_request(p_prev_request_id uuid, p_latitude numeric DEFAULT NULL::numeric, p_longitude numeric DEFAULT NULL::numeric)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Who may renew: the agent who filed it, the tenant's CURRENT handling agent
  -- (tenants get transferred), either side of a verified parent/sub-agent link,
  -- or ops/admin staff acting on an agent's behalf.
  v_allowed := (v_prev.agent_id = v_agent);
  ...
  IF NOT v_allowed THEN
    ...
    WHERE s.status = 'verified'
      AND (
        (s.parent_agent_id = v_agent AND s.sub_agent_id = v_prev.agent_id)
        OR (s.sub_agent_id = v_agent AND s.parent_agent_id = v_prev.agent_id)
      )
    LIMIT 1;
  END IF;
