-- 1. Extend the ledger category allowlist with the bonus→charge offset category
...
-- 2. Helper: how much of an agent's listing-rejection charges are still uncovered.
-- deficit = SUM(listing_rejection_penalty debits) - SUM(listing_rejection_offset credits),
-- floored at 0, counting only entries at/after the agent's wallet fresh-start anchor.
CREATE OR REPLACE FUNCTION public.get_agent_listing_rejection_deficit(p_agent_id uuid)
RETURNS numeric
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  ...
  SELECT
    COALESCE(SUM(CASE WHEN gl.category = 'listing_rejection_penalty' THEN gl.amount ELSE 0 END), 0),
    COALESCE(SUM(CASE WHEN gl.category = 'listing_rejection_offset'  THEN gl.amount ELSE 0 END), 0)
  INTO v_penalty, v_offset
  FROM public.general_ledger gl
  WHERE gl.user_id = p_agent_id
    AND gl.ledger_scope = 'wallet'
    AND gl.category IN ('listing_rejection_penalty','listing_rejection_offset')
    AND (gl.classification IS NULL OR gl.classification = 'production')
    AND (v_anchor_at IS NULL OR gl.transaction_date >= v_anchor_at);

  RETURN GREATEST(0, v_penalty - v_offset);
END;
$$;

GRANT EXECUTE ON FUNCTION public.get_agent_listing_rejection_deficit(uuid) TO authenticated, service_role;
