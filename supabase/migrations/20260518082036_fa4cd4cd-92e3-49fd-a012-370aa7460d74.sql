...
-- Allow new auto_match_method values
ALTER TABLE public.gmail_transactions
  DROP CONSTRAINT IF EXISTS gmail_transactions_auto_match_method_check;
ALTER TABLE public.gmail_transactions
  ADD CONSTRAINT gmail_transactions_auto_match_method_check
  CHECK (auto_match_method IN ('tid','amount','amount_strong'));

-- Drop old signature so we can change RETURNS TABLE
...
BEGIN
  ...
  LOOP
    ...
    -- 1. Exact TID match (highest priority, bulk-approvable)
    ...
    -- 2. Amount + extra-signal match. Score every candidate gmail row that
    --    shares this amount within the window, pick the highest score.
    --    Signals: name (counterparty/from_name vs profile.full_name),
    --             phone (digits-of-profile.phone appears in body/subject/counterparty/from_email),
    --             sender (from_email = profile.email),
    --             reference (deposit notes or transaction_id appears in body/subject).
    WITH cands AS (
      SELECT
        g.id,
        g.amount,
    ...
    scored AS (
      SELECT
        ...
        (
          CASE
            WHEN v_pending.full_name IS NOT NULL AND length(trim(v_pending.full_name)) >= 3
             AND (
               (c.counterparty IS NOT NULL AND lower(c.counterparty) LIKE '%' || lower(trim(v_pending.full_name)) || '%')
        ...
        (
          CASE
            WHEN length(v_phone_clean) >= 6
             AND (
               (c.raw_body IS NOT NULL AND regexp_replace(c.raw_body, '[^0-9]', '', 'g') LIKE '%' || v_phone_clean || '%')
        ...
        (
          CASE
            WHEN v_pending.email IS NOT NULL AND c.from_email IS NOT NULL
          ...
          CASE
            WHEN (
              (v_notes_clean IS NOT NULL AND length(v_notes_clean) >= 4
               AND (
                 (c.raw_body IS NOT NULL AND lower(c.raw_body) LIKE '%' || lower(v_notes_clean) || '%')
              ...
              OR (
                v_pending.transaction_id IS NOT NULL AND length(trim(v_pending.transaction_id)) >= 4
                AND (
                  (c.raw_body IS NOT NULL AND lower(c.raw_body) LIKE '%' || lower(trim(v_pending.transaction_id)) || '%')
    ...
    ranked AS (
      SELECT s.*, (s_name + s_phone + s_sender + s_reference) AS score
      FROM scored s
    )
    ...
    IF v_amt_match.id IS NOT NULL THEN
      -- Ambiguity guard: if another pending deposit also shares this amount in window,
      -- require this pending to have at least one extra signal AND to outscore alternatives.
      IF EXISTS (
        ...
        -- Pure-amount tie with no extra signal — skip (legacy safe behavior).
        CONTINUE;
      END IF;
