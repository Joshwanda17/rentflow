...
BEGIN
  ...
  -- Use exactly the same anchored ledger population as refresh_wallet_projection_for.
  -- Comparing against the unanchored all-time ledger can report delta=0 while the
  -- visible projection remains stale because pre-anchor rows are intentionally ignored.
  WITH anchor AS (
    SELECT a.anchor_at
    FROM public.wallet_fresh_start_anchors a
    WHERE a.user_id = v_desk_agent
    LIMIT 1
  )
  SELECT COALESCE(SUM(
    CASE
      WHEN g.direction IN ('cash_in','credit') THEN g.amount
      WHEN g.direction IN ('cash_out','debit') THEN -g.amount
      ELSE 0
    END
  ), 0)
    INTO v_visible_net
   ...
   WHERE g.user_id = v_desk_agent
     ...
     AND (a.anchor_at IS NULL OR g.created_at >= a.anchor_at);
