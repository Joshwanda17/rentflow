...
-- 1) Voluntary prepayment column: how many future daily installments are already covered.
ALTER TABLE public.agent_advances
  ADD COLUMN IF NOT EXISTS prepaid_installments_remaining INT NOT NULL DEFAULT 0;

-- 2) Rewrite sweep to (a) skip when today's ledger row already exists,
--    (b) consume the prepaid counter before actually sweeping,
--    (c) fire one aggregated SMS notification per agent via pg_net.
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      -- Skip if today already has any ledger row for this advance (cron ran,
      -- prior sweep ran, or voluntary prepayment covered today).
      SELECT EXISTS (
      ...
      -- Consume prepaid installment: mark today prepaid, decrement counter, skip actual sweep.
    ...
    -- Aggregated SMS notification per agent for this sweep (best-effort via pg_net).
