-- Resolve every non-archived auth user id whose profile OR auth phone matches
-- the given variants by last-9 digits. Mirrors get_email_by_phone matching so
-- that SMS password reset can update EVERY account the login flow could land
-- on (a phone can map to multiple UUIDs). Fixes "reset succeeds but new
-- password is rejected at sign-in".
...
-- service_role only: this is called from the password-reset-sms edge function
-- with the service key. Not exposed to anon/authenticated.
