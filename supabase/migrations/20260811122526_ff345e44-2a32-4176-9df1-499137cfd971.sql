ALTER TABLE public.merchandise_sales ADD COLUMN IF NOT EXISTS selected_size text;
...
BEGIN
  ...
  IF v_out > 0 THEN
    INSERT INTO public.merchandise_recovery_plans (
      customer_id, sale_id, item_name, original_amount, outstanding_balance,
      amount_recovered, status
    ) VALUES (
      v_uid, v_sale_id, v_item.item_name || COALESCE(' (' || v_size || ')', ''), v_total, v_out, v_down, 'active'
    );
  END IF;
...
DROP FUNCTION IF EXISTS public.agent_purchase_merchandise(uuid, integer, text);