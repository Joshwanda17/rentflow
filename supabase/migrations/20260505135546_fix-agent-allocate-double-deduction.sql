-- Fix double wallet deduction in agent_allocate_tenant_payment.
-- The general_ledger_route_buckets AFTER INSERT trigger already calls
-- apply_wallet_movement for every wallet-scope ledger row. The RPC was
-- additionally calling apply_wallet_movement explicitly, deducting float
-- (and crediting commission) twice and tripping wallets_balance_check
-- once cached float dropped below 2x the amount.
...
BEGIN
  ...
  -- Wallet-scope cash_out for float — the route_buckets trigger will
  -- apply the wallet bucket movement; do NOT call apply_wallet_movement
  -- explicitly here (that caused a double deduction).
  ...
  IF v_commission_earned > 0 THEN
    -- Wallet-scope cash_in for commission — bucket trigger routes to withdrawable.
    INSERT INTO public.general_ledger (
      user_id, amount, direction, category, source_table, source_id,
      description, ledger_scope, transaction_group_id, classification
    ) VALUES (
