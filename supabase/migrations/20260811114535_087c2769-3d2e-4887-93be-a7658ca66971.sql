CREATE OR REPLACE FUNCTION public.get_rent_disbursement_report(p_start timestamptz, p_end timestamptz)
RETURNS jsonb
LANGUAGE plpgsql
...
BEGIN
  ...
  WITH d AS (
    SELECT gl.id, gl.amount, gl.transaction_date, gl.source_id AS rent_request_id,
           gl.user_id AS tenant_id, gl.linked_party AS landlord_id, gl.description,
...
REVOKE ALL ON FUNCTION public.get_rent_disbursement_report(timestamptz, timestamptz) FROM public;
GRANT EXECUTE ON FUNCTION public.get_rent_disbursement_report(timestamptz, timestamptz) TO authenticated;