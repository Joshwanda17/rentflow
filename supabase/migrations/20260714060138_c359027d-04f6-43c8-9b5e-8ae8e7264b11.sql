...
BEGIN
  ...
  -- Record the operational collection so the Daily Capacity view
  -- (v_agent_daily_eligibility, sourced from agent_collections) counts
  -- this payment toward the agent's "paid_today" progress. Without this
  -- the ledger moved money but the capacity bar never advanced.
