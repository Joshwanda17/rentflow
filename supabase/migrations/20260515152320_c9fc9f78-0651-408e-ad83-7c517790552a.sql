-- ROLLBACK: restore previous production version of agent_allocate_tenant_payment
-- The revenue-recognition expansion attempted in the prior migration produced an
-- unbalanced ledger group (cash_in - cash_out off by principal_share), which is
-- rejected by create_ledger_transaction's unconditional balance check. Reverting
-- so production agent collections continue to work; revenue recognition will be
-- redesigned with a proper balancing leg in a follow-up.
