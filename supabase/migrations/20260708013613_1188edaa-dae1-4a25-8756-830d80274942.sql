...
BEGIN
  ...
  suggested AS (
    SELECT f.*,
      -- Suggested advance is derived STRICTLY from the potential score so the two
      -- always move together. A slightly super-linear curve (^1.4) keeps low
      -- scores small while rewarding high performers, capped at 30M and rounded
      -- to the nearest 50,000 UGX. Minimum 100,000 for any qualifying agent.
      GREATEST(100000,
        LEAST(30000000,
          ROUND(
            (power(LEAST(GREATEST(f.total_score, 0), 100) / 100.0, 1.4) * 30000000) / 50000.0
          ) * 50000
        )
      ) AS suggested_amt
    FROM finalized f
