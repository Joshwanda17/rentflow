...
BEGIN
  ...
  -- agents (incl. sub-agents) that have at least one rent request
  agent_ids AS (
  ...
  active_rents AS (
    SELECT COALESCE(daily_repayment, 0) AS daily_repayment,
           COALESCE(total_repayment, 0) AS total_repayment,
  ...
  expected_by_day AS (
    SELECT d.day,
           COALESCE((SELECT sum(ar.daily_repayment) FROM active_rents ar WHERE ar.start_day <= d.day), 0) AS expected
    FROM days d
  ),
  requested_by_day AS (
    SELECT (created_at AT TIME ZONE 'Africa/Kampala')::date AS day,
  ...
  outstanding_by_day AS (
    SELECT d.day,
           GREATEST(
             COALESCE((SELECT sum(ar.total_repayment) FROM active_rents ar WHERE ar.start_day <= d.day), 0)
