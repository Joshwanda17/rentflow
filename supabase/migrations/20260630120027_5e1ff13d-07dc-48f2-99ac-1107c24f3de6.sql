...
BEGIN
  ...
  IF v_payout.allocation_applied_id IS NOT NULL THEN
    UPDATE public.agent_landlord_float_allocations
    SET paid_out_amount = GREATEST(COALESCE(paid_out_amount, 0) - v_payout.amount, 0),
        remaining_amount = LEAST(
          COALESCE(allocated_amount, 0),
          COALESCE(remaining_amount, 0) + v_payout.amount
        ),
        status = CASE
          WHEN GREATEST(COALESCE(paid_out_amount, 0) - v_payout.amount, 0) <= 0 THEN 'open'
          ELSE 'partially_paid'
        END,
        updated_at = now()
    WHERE id = v_payout.allocation_applied_id;
  END IF;
  ...
  RETURN jsonb_build_object('ok', true, 'refunded', v_payout.amount, 'reason', p_reason, 'status', 'failed', 'allocation_reopened', v_payout.allocation_applied_id IS NOT NULL);
END;
$function$;