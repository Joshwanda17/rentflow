  ...
  LOOP
    ...
    ELSIF v_entry->>'direction' = 'cash_out' THEN
      ...
      IF NOT skip_balance_check AND COALESCE(v_entry->>'ledger_scope', 'wallet') = 'wallet' THEN
        -- Solvency must include CFO direct credits & corrections as real
        -- backing — same rule as get_user_available_balance and the
        -- end-user wallet helpers' balance math. (User-facing wallet *views*
        -- hide corrections per the user-facing-ledger-filter constraint, but
        -- balance computations MUST count them, otherwise legitimate CFO
        -- top-ups can't be withdrawn.)
        SELECT COALESCE(SUM(
