...
BEGIN
  ...
  WITH tenant_agg AS (
    SELECT
      ...
      COALESCE(SUM(
        CASE WHEN rr.status IN ('repaying','funded','disbursed')
              AND rr.amount_repaid < COALESCE(NULLIF(rr.total_repayment, 0), rr.rent_amount)
             THEN COALESCE(NULLIF(rr.total_repayment, 0), rr.rent_amount) - rr.amount_repaid
             ELSE 0 END), 0) AS overdue_amt
    FROM rent_requests rr
    WHERE rr.tenant_id IS NOT NULL
      AND rr.status NOT IN ('service_center_review','pending','rejected','cancelled','deleted_by_agent')
...
BEGIN
  ...
  overdue AS (
    SELECT
      rr.tenant_id AS uid,
      COALESCE(SUM(COALESCE(NULLIF(rr.total_repayment, 0), rr.rent_amount) - rr.amount_repaid), 0) AS current_overdue_amount
    FROM rent_requests rr
