-- Merchant (Cash-Out) Agents must hold the `process_cash_out` capability so
-- `can_process_cashout()` (and useAgentCapabilities().canCashOut) returns true
-- and the payout console fully unlocks. The CFO assignment flow only inserts a
-- `cashout_agents` row, so this capability was never granted — leaving active
-- merchant agents (e.g. Sharimah) unable to operate as merchants.

-- 1) Keep the capability in lock-step with the active cashout_agents row.
...
-- 2) Backfill: grant process_cash_out to every currently-active cashout agent.
