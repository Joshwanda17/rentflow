...
AS $function$
  ...
  WHERE auth.uid() IS NOT NULL
    ...
    -- Ordinary compounding returns stay hidden, but an explicit split payout
    -- has a separately approved cash portion that must remain payable.
    AND (
      COALESCE(ip.roi_mode, 'monthly_payout') NOT IN ('monthly_compounding', 'compound')
      OR pwo.operation_type = 'roi_split_cash'
    )
