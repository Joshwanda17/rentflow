...
-- Prefix index for national_id lookups (btree already exists, add text_pattern_ops for LIKE 'prefix%')
CREATE INDEX IF NOT EXISTS idx_profiles_national_id_prefix
  ON public.profiles (national_id text_pattern_ops)
  WHERE national_id IS NOT NULL AND national_id <> '';

CREATE OR REPLACE FUNCTION public.search_users_fast(
  p_query text,
...
BEGIN
  ...
  -- 3) National ID prefix — uses idx_profiles_national_id_prefix
  IF v_q ~* '^[A-Z0-9]{4,}$' THEN
    RETURN QUERY
      SELECT p.id, p.full_name, p.phone, p.email, p.national_id, p.verified
      FROM public.profiles p
...
REVOKE ALL ON FUNCTION public.search_users_fast(text, int) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.search_users_fast(text, int) TO authenticated;
