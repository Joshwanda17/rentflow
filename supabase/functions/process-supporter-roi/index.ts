 ...
 */
function effectiveNextRoiDateOnly(
  ...
  const todayMs = new Date(kampalaTodayDateOnly() + 'T00:00:00Z').getTime();
...
function isPortfolioRoiDue(
  p: { next_roi_date: string | null; created_at: string; payout_day: number | null },
): boolean {
  return effectiveNextRoiDateOnly(p.next_roi_date, p.created_at, p.payout_day) <= kampalaTodayDateOnly();
}
