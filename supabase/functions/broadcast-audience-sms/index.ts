...
async function sendViaYoola(phone: string, message: string): Promise<SmsResult> {
  ...
  try {
    const res = await fetch('https://yoolasms.com/api/v1/send', {
      ...
      body: JSON.stringify({ phone: toBareDigits(phone), sender_id: "WELILE", message, api_key: apiKey, sender: "WELILE"}),
    });
...
async function sendViaAfricasTalking(phone: string, message: string): Promise<SmsResult> {
  ...
  const params = new URLSearchParams({ username, to: formatPhoneInternational(phone), from: "WELILE", message });
  try {
