import type { CalculationResult, PayComponentInput, PayslipLine, RuleVersion } from './types';
...
export function calculatePayslip(
  ...
  const nssfEmployee = computeNssfEmployee(nssfableGross, rule);
  const nssfEmployer = computeNssfEmployer(nssfableGross, rule);

  const chargeableIncome = rule.nssfReducesPayeBase ? taxableGross - nssfEmployee : taxableGross;

  const paye = computePaye(chargeableIncome, rule);
  const lst = lstMonthly;
  ...
  const band = chargeableIncome > 0 ? selectBand(chargeableIncome, rule) : null;
  const payeSentence = band
