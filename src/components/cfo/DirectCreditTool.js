...
import { PayoutAutomationToggle, describeSchedule } from './PayoutAutomationToggle';
...
export function DirectCreditTool() {
    ...
    const [locationRecipients, setLocationRecipients] = useState([]);
    ...
    const [automateConfig, setAutomateConfig] = useState({
        frequency: 'monthly',
        dayOfMonth: 1,
        dayOfWeek: 1,
        intervalDays: 7,
    ...
    // allow_overdraw=true.
    const [overdrawInfo, setOverdrawInfo] = useState(null);
    ...
    const availableCategories = useMemo(() => {
        if (operation === 'withdraw') {
            ...
            return PAYOUT_CATEGORIES.filter(c => c.impact === 'expense' && c.allowedOps.includes('credit'));
        }
        return PAYOUT_CATEGORIES.filter(c => c.allowedOps.includes(operation));
    }, [operation]);
    const selectedCategory = useMemo(() => PAYOUT_CATEGORIES.find(c => c.id === selectedCategoryId), [selectedCategoryId]);
    ...
    const selectedSubCategory = useMemo(() => selectedCategory?.subCategories?.find(s => s.id === selectedSubCategoryId), [selectedCategory, selectedSubCategoryId]);
    ...
    const rentRequestIdsFromLocation = useMemo(() => locationRecipients.map(r => r.rent_request_id).filter(Boolean), [locationRecipients]);
    ...
    const mutation = useMutation({
        mutationFn: async (opts) => {
            ...
            const targets = locationRecipients.length > 0 ? locationRecipients : [selectedUser];
            if (!selectedCategory)
                throw new Error('Select a payout category');
            ...
            if (operation === 'withdraw') {
                if (selectedCategory.impact !== 'expense') {
                    throw new Error(`Withdrawal blocked: only Expense categories may be withdrawn. "${selectedCategory.label}" is marked as ${selectedCategory.impact}.`);
                }
                if (payoutMethod === 'cash') {
                    ...
                    if (momoNumber.trim().length < 9)
                        throw new Error('Mobile Money number is required (min 9 digits).');
                    if (momoName.trim().length < 2)
                    ...
                    if (bankAccountNumber.trim().length < 5)
            ...
            const effectiveRecipient = selectedCategory.recipientLock !== 'either'
            ...
            const submitOperation = operation === 'withdraw' ? 'credit' : operation;
            ...
            const postOne = async (target) => {
                const { data, error } = await supabase.functions.invoke('cfo-direct-credit', {
                    body: {
                        ...
                        solvency_bypass_reason: opts?.allowOverdraw === true && effectiveRecipient !== 'operational_wallet'
                            ? 'duplicate_reversal'
                            : undefined,
                    },
                });
                if (error) {
                    ...
                    if (msg.includes('INSUFFICIENT_AVAILABLE_BALANCE') ||
                        ...
                        const silent = new Error('BUCKET_INSUFFICIENT_BALANCE');
                    ...
                    if (msg.includes('Unauthorized'))
                        throw new Error('You do not have permission. Please log in again.');
                    if (msg.includes('Insufficient permissions'))
                        throw new Error('Your role does not have CFO privileges.');
                    ...
                    if (msg.includes('Insufficient ledger balance'))
                        throw new Error(msg);
                    if (msg.includes('INVALID_ROUTING'))
                        throw new Error(msg.replace(/^.*INVALID_ROUTING:\s*/, 'Routing rejected: '));
                    if (msg.includes('RECIPIENT_TYPE_REQUIRED'))
                        throw new Error('Choose a Recipient type — User (Withdrawable) or Operational Wallet (Float).');
                    if (msg.includes('NEGATIVE_WALLET_BLOCKED')) {
                        ...
                        throw new Error(`Debit refused: this user's strict withdrawable balance is UGX ${avail}. ` +
            ...
            if (targets.length <= 1) {
                ...
                setOverdrawApproved(false);
                if (ok === 0)
                    throw new Error(failures[0] ?? 'Payout failed');
                resultData = {
            ...
            if (automateEnabled && selectedUser && targets.length <= 1) {
                ...
                if (schedErr) {
                    ...
                    await logStandingOrderAction({
                        scheduledPayoutId: schedRow?.id ?? null,
                        action: 'create',
                    ...
                    try {
                        await supabase.functions.invoke('notify-standing-order-setup', {
                            body: {
                                target_user_id: selectedUser.id,
                                scheduled_payout_id: schedRow?.id ?? null,
                                amount: amt,
    ...
    const handleFreezeAndSplitDebit = async () => {
        if (!overdrawInfo || !selectedUser || !selectedCategory)
            return;
        const effectiveRecipient = selectedCategory.recipientLock !== 'either'
            ? selectedCategory.recipientLock
            : recipientType;
        ...
        if (effectiveRecipient === 'operational_wallet' || overdrawInfo.bucket === 'float') {
            toast({
                ...
                description: 'This correction affects operational float only. No personal debt may be created — lower the amount or allocate more float first.',
                variant: 'destructive',
            });
            return;
        }
        ...
        try {
            ...
            if (clean > 0) {
                ...
                if (data?.error)
                    throw new Error(`Clean debit failed: ${data.error}`);
            }
            ...
            if (debt > 0) {
                ...
                if (data?.error)
                    throw new Error(`Force reversal failed: ${data.error}`);
            }
            ...
            if (freezeErr)
                throw new Error(`Debits posted but freeze failed: ${freezeErr.message}`);
            toast({
    ...
    return (<Card>
      ...
      <CardContent className="space-y-3">
        <div className="grid grid-cols-3 gap-2">
          <Button type="button" variant={isCredit ? 'default' : 'outline'} className={isCredit ? 'bg-emerald-600 hover:bg-emerald-700 text-white' : ''} onClick={() => handleOperationChange('credit')}>
            <ArrowUpRight className="h-4 w-4 mr-1.5 shrink-0"/>
            <span>{CFO_PAYOUT_LABELS.credit}</span>
          </Button>
          <Button type="button" variant={operation === 'debit' ? 'default' : 'outline'} className={operation === 'debit' ? 'bg-destructive hover:bg-destructive/90 text-destructive-foreground' : ''} onClick={() => handleOperationChange('debit')}>
            <ArrowDownLeft className="h-4 w-4 mr-1.5 shrink-0"/>
            <span>{CFO_PAYOUT_LABELS.debit}</span>
          </Button>
          <Button type="button" variant={operation === 'withdraw' ? 'default' : 'outline'} className={operation === 'withdraw' ? 'bg-orange-600 hover:bg-orange-700 text-white' : ''} onClick={() => handleOperationChange('withdraw')}>
            <Banknote className="h-4 w-4 mr-1.5 shrink-0"/>
            <span>Withdraw</span>
          </Button>
        </div>

        {operation === 'withdraw' && (<div className="rounded-lg border border-orange-200 bg-orange-50 p-3 text-[11px] text-orange-800 flex gap-2">
        ...
        {operation === 'debit' && (<RecipientRoutingWarningBanner />)}
        {operation === 'credit' && (<RecipientRoutingWarningBanner variant="compact"/>)}
        ...
        <div>
          ...
          <div className="relative">
            <select id="payout-category" value={selectedCategoryId} onChange={(e) => handleCategoryChange(e.target.value)} className="flex h-10 w-full appearance-none rounded-lg border border-border bg-background px-3 py-2 pr-10 text-sm text-foreground transition-colors ring-offset-background focus:outline-none focus:ring-2 focus:ring-ring/20 focus:border-primary/50">
        ...
        {hasSubCategories && (<div>
            ...
            <div className="relative">
              <select id="payout-subcategory" value={selectedSubCategoryId} onChange={(e) => setSelectedSubCategoryId(e.target.value)} className="flex h-10 w-full appearance-none rounded-lg border border-border bg-background px-3 py-2 pr-10 text-sm text-foreground transition-colors ring-offset-background focus:outline-none focus:ring-2 focus:ring-ring/20 focus:border-primary/50">
                <option value="">Select a subcategory...</option>
                {selectedCategory?.subCategories?.map((sub) => (<option key={sub.id} value={sub.id}>
                    {sub.label}
                  </option>))}
              </select>
              <ChevronDown className="pointer-events-none absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground"/>
        ...
        {selectedCategory && impactInfo && ImpactIcon && !isQueueCategory && !needsSubCategory && (<div className={`rounded-lg border p-3 text-xs space-y-1.5 ${impactInfo.color}`}>
            ...
            {selectedSubCategory && (<p className="font-medium">📂 Subcategory: {selectedSubCategory.label}</p>)}
            <div className="pt-1 border-t border-current/10 space-y-0.5 text-[10px]">
              ...
              {selectedCategory.impact === 'expense' && (<p className="font-medium text-orange-700">
                  📉 This payout is a platform expense — reduces platform earnings.
                </p>)}
              {selectedCategory.impact === 'revenue' && (<p className="font-medium text-emerald-700">
                  ✅ This payout earns money for the platform — recorded as platform income.
                </p>)}
              {selectedCategory.impact === 'neutral' && (<p className="font-medium">
        ...
        {isRentDisbursement && (<>
            {locationRecipients.length > 0 && (<div className="rounded-lg border border-primary/30 bg-primary/5 p-3 text-xs space-y-1">
                ...
                <button type="button" className="text-primary hover:underline" onClick={() => setLocationRecipients([])}>
                  Show the full queue again
                </button>
              </div>)}
            <RentDisbursementQueue restrictToIds={rentRequestIdsFromLocation} autoSelectIds={rentRequestIdsFromLocation}/>
          </>)}

        {/* ── BUSINESS ADVANCE DISBURSEMENT QUEUE ── */}
        {isBusinessAdvance && locationRecipients.length === 0 && (<div className="space-y-4">
            <CreditDrawApprovalQueue />
            <BusinessAdvanceDisbursementQueue />
          </div>)}

        {/* ── ROI PAYOUT QUEUE ── */}
        {isROIPayout && locationRecipients.length === 0 && (<ROIPayoutQueue />)}
        ...
        {(!isQueueCategory || (locationRecipients.length > 0 && !isRentDisbursement)) && selectedCategoryId && !needsSubCategory && (<>
            ...
            <UserSearchPicker label="Search User" placeholder="Name or phone..." selectedUser={selectedUser} onSelect={(u) => { setLocationRecipients([]); setSelectedUser(u); }}/>
            ...
            {selectedUser && (<div>
                <Label className="flex items-center gap-1.5 mb-1.5">
                  Recipient Type
                  <span className="text-destructive">*</span>
                  {selectedCategory && selectedCategory.recipientLock !== 'either' && (<Badge variant="outline" className="ml-1 text-[9px] px-1.5 py-0 h-4">
                      Auto-set by category
                    </Badge>)}
                </Label>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  <button type="button" onClick={() => {
                    if (selectedCategory?.recipientLock === 'operational_wallet')
                        return;
                    setRecipientType('user');
                }} disabled={selectedCategory?.recipientLock === 'operational_wallet'} className={`text-left rounded-lg border p-3 transition ${recipientType === 'user'
                    ? 'border-emerald-500 bg-emerald-50 ring-2 ring-emerald-200'
                    : 'border-border hover:border-emerald-300 hover:bg-emerald-50/40'} ${selectedCategory?.recipientLock === 'operational_wallet' ? 'opacity-40 cursor-not-allowed hover:bg-transparent hover:border-border' : ''}`}>
                  ...
                  <button type="button" onClick={() => {
                    if (selectedCategory?.recipientLock === 'user')
                        return;
                    setRecipientType('operational_wallet');
                }} disabled={selectedCategory?.recipientLock === 'user'} className={`text-left rounded-lg border p-3 transition ${recipientType === 'operational_wallet'
                    ? 'border-amber-500 bg-amber-50 ring-2 ring-amber-200'
                    : 'border-border hover:border-amber-300 hover:bg-amber-50/40'} ${selectedCategory?.recipientLock === 'user' ? 'opacity-40 cursor-not-allowed hover:bg-transparent hover:border-border' : ''}`}>
                ...
                {selectedCategory && selectedCategory.recipientLock !== 'either' ? (<p className="text-[10px] text-muted-foreground mt-1.5">
                  ...
                  </p>) : (<p className="text-[10px] text-muted-foreground mt-1.5">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5 mt-2">
                {[10000, 50000, 100000, 200000, 500000].map(v => (<Button key={v} size="sm" variant="outline" className="text-xs h-7" onClick={() => setAmount(String(v))}>
                    {(v / 1000).toFixed(0)}K
                  </Button>))}
              </div>
            </div>
            ...
            {operation === 'withdraw' && (<div className="space-y-3 rounded-lg border border-orange-200 bg-orange-50/40 p-3">
                <div>
                  <Label className="flex items-center gap-1.5 mb-1.5">
                    Payout Method <span className="text-destructive">*</span>
                  </Label>
                  <div className="grid grid-cols-3 gap-2">
                    <button type="button" onClick={() => setPayoutMethod('cash')} className={`rounded-lg border p-2 text-xs font-medium transition flex flex-col items-center gap-1 ${payoutMethod === 'cash'
                    ? 'border-orange-500 bg-white ring-2 ring-orange-200'
                    : 'border-border bg-white hover:border-orange-300'}`}>
                      <Banknote className="h-4 w-4"/>
                      Cash
                    </button>
                    <button type="button" onClick={() => setPayoutMethod('mobile_money')} className={`rounded-lg border p-2 text-xs font-medium transition flex flex-col items-center gap-1 ${payoutMethod === 'mobile_money'
                    ? 'border-orange-500 bg-white ring-2 ring-orange-200'
                    : 'border-border bg-white hover:border-orange-300'}`}>
                      <Phone className="h-4 w-4"/>
                      MoMo
                    </button>
                    <button type="button" onClick={() => setPayoutMethod('bank_transfer')} className={`rounded-lg border p-2 text-xs font-medium transition flex flex-col items-center gap-1 ${payoutMethod === 'bank_transfer'
                    ? 'border-orange-500 bg-white ring-2 ring-orange-200'
                    : 'border-border bg-white hover:border-orange-300'}`}>
                ...
                {payoutMethod === 'cash' && (<div>
                    <Label>Pickup Details <span className="text-destructive">*</span></Label>
                    <Input placeholder="e.g. Welile HQ Reception, Nakasero — collected by Jane Doe" value={pickupLocation} onChange={(e) => setPickupLocation(e.target.value)}/>
                  </div>)}

                {payoutMethod === 'mobile_money' && (<div className="grid grid-cols-2 gap-2">
                    <div className="col-span-2">
                      <Label>MoMo Number <span className="text-destructive">*</span></Label>
                      <Input type="tel" placeholder="07XX XXX XXX" value={momoNumber} onChange={(e) => setMomoNumber(e.target.value)}/>
                    </div>
                    <div>
                      <Label>Provider <span className="text-destructive">*</span></Label>
                      <div className="relative">
                        <select value={momoProvider} onChange={(e) => setMomoProvider(e.target.value)} className="flex h-10 w-full appearance-none rounded-lg border border-border bg-background px-3 py-2 pr-10 text-sm">
                    ...
                    <div>
                      ...
                      <Input placeholder="Registered MoMo name" value={momoName} onChange={(e) => setMomoName(e.target.value)}/>
                    </div>
                  </div>)}

                {payoutMethod === 'bank_transfer' && (<div className="grid grid-cols-2 gap-2">
                    <div className="col-span-2">
                      <Label>Bank Name <span className="text-destructive">*</span></Label>
                      <div className="relative">
                        <select value={bankName} onChange={(e) => setBankName(e.target.value)} className="flex h-10 w-full appearance-none rounded-lg border border-border bg-background px-3 py-2 pr-10 text-sm">
                    ...
                    <div>
                      <Label>Account Name <span className="text-destructive">*</span></Label>
                      <Input placeholder="Account holder full name" value={bankAccountName} onChange={(e) => setBankAccountName(e.target.value)}/>
                    </div>
                  </div>)}
              </div>)}
            ...
            <div>
              ...
              <Textarea placeholder={operation === 'withdraw'
                ? 'e.g. April marketing campaign payout to vendor...'
                : isCredit
                    ? 'e.g. ROI payout for March cycle, salary advance for agent...'
                    : 'e.g. Access fee collection, penalty for policy violation...'} value={reason} onChange={e => setReason(e.target.value)} rows={2}/>
              <p className="text-[10px] text-muted-foreground mt-1">{reason.length}/10 characters minimum</p>
            </div>

            {/* Automation Toggle (credit only) */}
            {isCredit && selectedUser && (<PayoutAutomationToggle enabled={automateEnabled} onToggle={setAutomateEnabled} config={automateConfig} onConfigChange={setAutomateConfig}/>)}

            {/* Treasury Impact */}
            {amt > 0 && isCredit && (<TreasuryImpactBanner payoutAmount={amt}/>)}

            {/* Summary before submit */}
            {selectedCategory && amt > 0 && selectedUser && recipientType && (<div className="rounded-lg bg-muted/30 border p-3 text-xs space-y-1">
                ...
                {isCredit && (() => {
                    const bucket = RECIPIENT_BUCKET[recipientType];
                    const meta = BUCKET_LABEL[bucket];
                    return (<div className={`mt-2 rounded-md border p-2 text-[11px] ${meta.tone}`}>
            ...
            <Button className={`w-full ${operation === 'withdraw'
                ? 'bg-orange-600 hover:bg-orange-700'
                : isCredit
                    ? 'bg-emerald-600 hover:bg-emerald-700'
                    : 'bg-destructive hover:bg-destructive/90'}`} onClick={() => {
              ...
              <AlertDialogContent>
                ...
                : selectedUser?.full_name || '...'}
                      ...
                      <label className="flex items-start gap-2 rounded-md border border-border p-3 mt-2 cursor-pointer">
                        <Checkbox checked={floatApproved} onCheckedChange={(v) => setFloatApproved(v === true)} disabled={mutation.isPending} className="mt-0.5"/>
                ...
                <AlertDialogFooter>
                  ...
                  <AlertDialogAction className="bg-emerald-600 hover:bg-emerald-700" onClick={(e) => {
                e.preventDefault();
            ...
            <AlertDialog open={!!overdrawInfo} onOpenChange={(o) => { if (!o) {
            setOverdrawInfo(null);
            setOverdrawApproved(false);
        } }}>
              <AlertDialogContent>
                ...
                : "The user's live ledger balance is not enough to cover this correction."}
                      ...
                      {overdrawInfo?.bucket === 'float' ? (<div className="rounded-md border border-emerald-200 bg-emerald-50 p-3 space-y-2 text-foreground">
                      ...
                      <label className="flex items-start gap-2 rounded-md border border-amber-200 bg-amber-50 p-3 cursor-pointer">
                        <Checkbox checked={overdrawApproved} onCheckedChange={(v) => setOverdrawApproved(v === true)} disabled={mutation.isPending || splitFreezing} className="mt-0.5"/>
                        <span className="text-foreground">
                ...
                <AlertDialogFooter>
                  ...
                  {overdrawInfo?.bucket !== 'float' && (<>
                  <Button type="button" className="bg-red-600 hover:bg-red-700 text-white" onClick={handleFreezeAndSplitDebit} disabled={mutation.isPending || splitFreezing || !overdrawApproved}>
                    {splitFreezing ? <Loader2 className="h-4 w-4 animate-spin mr-2"/> : null}
                    Freeze + Split Debit
                  </Button>
                  <AlertDialogAction className="bg-amber-600 hover:bg-amber-700" onClick={(e) => {
                    e.preventDefault();
