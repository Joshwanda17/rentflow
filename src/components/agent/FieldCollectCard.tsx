...
export function FieldCollectCard({ onOpen }: FieldCollectCardProps) {
  ...
  return (
    <button
      ...
      className="w-full text-left rounded-3xl border bg-card hover:bg-accent/40 active:bg-accent/60 transition-colors touch-manipulation flex items-center gap-4 min-h-[96px] px-5 py-4"
    ...
    >
      <div className="min-w-0 flex-1">
        ...
        {hasActivity ? (
          <p className="text-[28px] font-bold tabular-nums leading-tight tracking-tight mt-0.5">
            {formatUGX(total)}
          </p>
        ) : (
          <p className="text-base font-semibold mt-0.5">Tap to record cash</p>
        )}
        {needsAttention && (
