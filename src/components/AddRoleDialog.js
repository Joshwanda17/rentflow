...
import { Plus, Home, Users, Wallet, Building2, Shield } from 'lucide-react';
...
export default function AddRoleDialog({ availableRoles, onAddRole }) {
    ...
    const addRoleDirectly = async (role) => {
        setIsLoading(true);
    ...
    return (<Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="max-w-md">
        ...
        {showCodeInput ? (<div className="space-y-4 pt-4">
            ...
            <div className="space-y-2">
              ...
              <Input type="password" placeholder="Enter access code" value={accessCode} onChange={(e) => setAccessCode(e.target.value)}/>
            </div>
            ...
            {!canEdit && (<p className="text-sm text-muted-foreground">
                You can add standard roles here. Only access administrators can add internal and executive roles.
              </p>)}
            {missingRoles.map((role) => (<button key={role.value} onClick={() => handleRoleSelect(role.value)} disabled={isLoading || (!canEdit && !['tenant', 'agent', 'landlord', 'supporter', 'senior_agent', 'sub_agent'].includes(role.value))} className="w-full flex items-center gap-3 p-3 rounded-lg border border-border hover:border-primary hover:bg-primary/5 transition-all text-left">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    {role.requiresCode && (<span className="text-xs px-2 py-0.5 rounded bg-warning/10 text-warning">
                        Code Required
                      </span>)}
                  </div>
