import { useState, useEffect, useCallback, useMemo } from 'react';
import { useQuery, keepPreviousData } from '@tanstack/react-query';
import { supabase } from '@/integrations/supabase/client';
import { Input } from '@/components/ui/input';
import { Badge } from '@/components/ui/badge';
import { Button } from '@/components/ui/button';
import { KPICard } from './KPICard';
import { UserProfileDialog } from '@/components/supporter/UserProfileDialog';
import {
  Search, Building2, X, ShieldCheck, Home, Sparkles,
  ChevronLeft, ChevronRight, Phone, MapPin, Loader2, Landmark, Smartphone,
} from 'lucide-react';
import { cn } from '@/lib/utils';
import { format } from 'date-fns';

interface LandlordRow {
  id: string;
  name: string | null;
  phone: string | null;
  verified: boolean;
  created_at: string | null;
  country: string | null;
  region: string | null;
  district: string | null;
  city: string | null;
  property_address: string | null;
  bank_name: string | null;
  account_number: string | null;
  mobile_money_number: string | null;
  mobile_money_name: string | null;
  number_of_houses: number | null;
  monthly_rent: number | null;
  is_occupied: boolean | null;
  is_agent_managed: boolean | null;
  verification_status: string | null;
  total_matched: number;
}

interface Totals {
  total: number;
  verified: number;
  occupied: number;
  new30d: number;
}

const PAGE_SIZE = 50;

const fmt = (n: number) =>
  n >= 1e6 ? `${(n / 1e6).toFixed(1)}M` : n >= 1e3 ? `${(n / 1e3).toFixed(1)}K` : n.toLocaleString();

const fmtUgx = (n: number | null) =>
  n == null ? '—' : `UGX ${Number(n).toLocaleString()}`;

interface CRMLandlordsPanelProps {
  title: string;
  subtitle: string;
}

export function CRMLandlordsPanel({ title, subtitle }: CRMLandlordsPanelProps) {
  const [searchInput, setSearchInput] = useState('');
  const [search, setSearch] = useState('');
  const [page, setPage] = useState(0);
  const [selected, setSelected] = useState<LandlordRow | null>(null);

  useEffect(() => {
    const t = setTimeout(() => {
      setSearch(searchInput.trim());
      setPage(0);
    }, 300);
    return () => clearTimeout(t);
  }, [searchInput]);

  const { data: totals } = useQuery<Totals>({
    queryKey: ['crm-landlords-totals'],
    queryFn: async () => {
      const { data, error } = await supabase.rpc('get_crm_landlords_totals');
      if (error) throw error;
      const row = (data as any[])?.[0];
      return {
        total: Number(row?.total ?? 0),
        verified: Number(row?.verified ?? 0),
        occupied: Number(row?.occupied ?? 0),
        new30d: Number(row?.new30d ?? 0),
      };
    },
    staleTime: 120_000,
  });

  const { data: rows, isLoading, isFetching } = useQuery<LandlordRow[]>({
    queryKey: ['crm-landlords-rows', search, page],
    queryFn: async () => {
      const { data, error } = await supabase.rpc('get_crm_landlords', {
        _search: search || null,
        _limit: PAGE_SIZE,
        _offset: page * PAGE_SIZE,
      });
      if (error) throw error;
      return (data as LandlordRow[]) || [];
    },
    staleTime: 60_000,
    placeholderData: keepPreviousData,
  });

  const list = rows ?? [];
  const totalMatched = list[0]?.total_matched ?? 0;
  const totalPages = Math.max(1, Math.ceil(Number(totalMatched) / PAGE_SIZE));

  const openProfile = useCallback((r: LandlordRow) => setSelected(r), []);

  const profileUser = useMemo(() => {
    if (!selected) return null;
    return {
      id: selected.id,
      name: selected.name || 'Unknown Landlord',
      type: 'landlord' as const,
      createdAt: selected.created_at ?? undefined,
      phone: selected.phone ?? undefined,
      verified: selected.verified,
      propertyAddress: selected.property_address ?? undefined,
      numberOfHouses: selected.number_of_houses ?? undefined,
      desiredRent: selected.monthly_rent ?? undefined,
      city: selected.city || selected.district || undefined,
      country: selected.country ?? undefined,
    };
  }, [selected]);

  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-lg font-bold tracking-tight">{title}</h2>
        <p className="text-sm text-muted-foreground">{subtitle}</p>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <KPICard title="Total Landlords" value={fmt(totals?.total ?? 0)} icon={Building2} />
        <KPICard title="Verified" value={fmt(totals?.verified ?? 0)} icon={ShieldCheck} color="bg-emerald-500/10 text-emerald-600" />
        <KPICard title="Occupied" value={fmt(totals?.occupied ?? 0)} icon={Home} color="bg-blue-500/10 text-blue-600" />
        <KPICard title="New (30d)" value={fmt(totals?.new30d ?? 0)} icon={Sparkles} color="bg-purple-500/10 text-purple-600" />
      </div>

      {/* Search */}
      <div className="relative max-w-md">
        <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
        <Input
          value={searchInput}
          onChange={(e) => setSearchInput(e.target.value)}
          placeholder="Search by name, phone, location, bank, or mobile money…"
          className="pl-9 pr-9"
        />
        {searchInput && (
          <button
            onClick={() => setSearchInput('')}
            className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground"
            aria-label="Clear search"
          >
            <X className="h-4 w-4" />
          </button>
        )}
      </div>

      {/* Table */}
      <div className="border border-border rounded-xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-muted/50 border-b border-border text-left">
                <th className="px-3 py-2.5 font-semibold text-muted-foreground whitespace-nowrap">Name</th>
                <th className="px-3 py-2.5 font-semibold text-muted-foreground whitespace-nowrap">Phone</th>
                <th className="px-3 py-2.5 font-semibold text-muted-foreground whitespace-nowrap">Location</th>
                <th className="px-3 py-2.5 font-semibold text-muted-foreground whitespace-nowrap">Payout</th>
                <th className="px-3 py-2.5 font-semibold text-muted-foreground whitespace-nowrap">Houses</th>
                <th className="px-3 py-2.5 font-semibold text-muted-foreground whitespace-nowrap">Monthly Rent</th>
                <th className="px-3 py-2.5 font-semibold text-muted-foreground whitespace-nowrap">Occupied</th>
                <th className="px-3 py-2.5 font-semibold text-muted-foreground whitespace-nowrap">Verified</th>
                <th className="px-3 py-2.5 font-semibold text-muted-foreground whitespace-nowrap">Joined</th>
              </tr>
            </thead>
            <tbody>
              {isLoading ? (
                Array.from({ length: 8 }).map((_, i) => (
                  <tr key={i} className="border-b border-border">
                    {Array.from({ length: 9 }).map((__, j) => (
                      <td key={j} className="px-3 py-3"><div className="h-4 w-20 bg-muted animate-pulse rounded" /></td>
                    ))}
                  </tr>
                ))
              ) : list.length === 0 ? (
                <tr>
                  <td colSpan={9} className="px-3 py-10 text-center text-muted-foreground">No landlords found</td>
                </tr>
              ) : (
                list.map((r) => (
                  <tr
                    key={r.id}
                    onClick={() => openProfile(r)}
                    className="border-b border-border last:border-0 hover:bg-muted/30 transition-colors cursor-pointer"
                  >
                    <td className="px-3 py-2.5 font-medium whitespace-nowrap">
                      {r.name || '—'}
                      {r.is_agent_managed && (
                        <Badge variant="secondary" className="ml-2 text-[10px]">Agent-managed</Badge>
                      )}
                    </td>
                    <td className="px-3 py-2.5 whitespace-nowrap">
                      {r.phone ? (
                        <span className="flex items-center gap-1 text-muted-foreground"><Phone className="h-3 w-3" />{r.phone}</span>
                      ) : '—'}
                    </td>
                    <td className="px-3 py-2.5 whitespace-nowrap text-muted-foreground">
                      <span className="flex items-center gap-1">
                        {(r.city || r.district || r.region) && <MapPin className="h-3 w-3" />}
                        {[r.city, r.district, r.region].filter(Boolean).join(', ') || '—'}
                      </span>
                    </td>
                    <td className="px-3 py-2.5 whitespace-nowrap text-muted-foreground">
                      {r.bank_name ? (
                        <span className="flex items-center gap-1"><Landmark className="h-3 w-3" />{r.bank_name}</span>
                      ) : r.mobile_money_number ? (
                        <span className="flex items-center gap-1"><Smartphone className="h-3 w-3" />{r.mobile_money_number}</span>
                      ) : '—'}
                    </td>
                    <td className="px-3 py-2.5 whitespace-nowrap text-muted-foreground">{r.number_of_houses ?? '—'}</td>
                    <td className="px-3 py-2.5 whitespace-nowrap text-muted-foreground">{fmtUgx(r.monthly_rent)}</td>
                    <td className="px-3 py-2.5 whitespace-nowrap">
                      {r.is_occupied
                        ? <Badge className="bg-blue-500/10 text-blue-600 border-blue-500/30 text-xs">Occupied</Badge>
                        : <span className="text-muted-foreground text-xs">Vacant</span>}
                    </td>
                    <td className="px-3 py-2.5 whitespace-nowrap">
                      {r.verified
                        ? <Badge className="bg-emerald-500/10 text-emerald-600 border-emerald-500/30 text-xs">Verified</Badge>
                        : <span className="text-muted-foreground text-xs">—</span>}
                    </td>
                    <td className="px-3 py-2.5 whitespace-nowrap text-muted-foreground">
                      {r.created_at ? format(new Date(r.created_at), 'dd MMM yy') : '—'}
                    </td>
                  </tr>
                ))
              )}
            </tbody>
          </table>
        </div>
      </div>

      {/* Pagination */}
      <div className="flex items-center justify-between gap-3">
        <p className="text-xs text-muted-foreground flex items-center gap-2">
          {isFetching && <Loader2 className="h-3 w-3 animate-spin" />}
          {Number(totalMatched).toLocaleString()} landlords
          {totalMatched > 0 && ` · page ${page + 1} of ${totalPages}`}
        </p>
        <div className="flex gap-1.5">
          <Button variant="outline" size="sm" className="h-8" disabled={page === 0} onClick={() => setPage((p) => Math.max(0, p - 1))}>
            <ChevronLeft className="h-4 w-4" /> Prev
          </Button>
          <Button variant="outline" size="sm" className="h-8" disabled={page + 1 >= totalPages} onClick={() => setPage((p) => p + 1)}>
            Next <ChevronRight className="h-4 w-4" />
          </Button>
        </div>
      </div>

      <UserProfileDialog open={!!selected} onOpenChange={(o) => !o && setSelected(null)} user={profileUser as any} />
    </div>
  );
}