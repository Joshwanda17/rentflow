...
import {
  ChevronRight, MapPin, Loader2, Home, Users, Wallet, UserCheck, Sparkles, Search,
  Calendar as CalendarIcon, X, Filter, Globe2, ArrowLeft, RefreshCw,
} from 'lucide-react';
...
export function GeographicCoveragePanel() {
  ...
  // ---- Scope state (lives inside the sheet, but we also use it for the main card totals) ----
  ...
  const p_to = toDate ? new Date(toDate.getTime() + 86_399_000).toISOString() : null;
  ...
  const { data: rows = [], isLoading, isFetching, refetch } = useQuery({
    ...
    enabled: drillOpen, // only fetch when the sheet is open
  });
  ...
  return (
    <TooltipProvider delayDuration={200}>
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          <Button
            ...
            className="w-full h-auto py-4 gap-2 text-left justify-start"
            variant="outline"
          >
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <div className="font-semibold text-sm">Tenants whose landlord was funded</div>
                <div className="text-xs text-muted-foreground">
                  Drill through Country → District → City to see funded tenants
                </div>
              </div>
      ...
      {/* ====== UNIFIED SHEET: Geo drill-down + Funded tenants ====== */}
      <Sheet open={drillOpen} onOpenChange={setDrillOpen}>
        <SheetContent className="w-full sm:max-w-4xl overflow-y-auto flex flex-col">
          ...
          <div className="mt-3 space-y-3 flex-1">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <DateRangePopover label="From" date={fromDate} onChange={setFromDate} />
              <DateRangePopover label="To" date={toDate} onChange={setToDate} />
            ...
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
              ...
              <div className="rounded-md border bg-primary/5 p-2 text-left">
            ...
            <div className="border-t pt-3">
              <div className="flex items-center gap-2 mb-2">
                ...
                <h4 className="text-sm font-semibold">
                  Funded tenants in {scopeLabel}
                </h4>
                <Badge variant="secondary" className="text-[10px]">{fmt(fundedTotal)}</Badge>
              </div>
            </div>

            {/* Funded tenant list */}
            {loadingFunded ? (
              <div className="border rounded-md" role="status" aria-label="Loading funded tenants">
              ...
              <div className="py-6 text-center text-sm text-muted-foreground border rounded-md">No funded tenants in this area.</div>
            ) : (
