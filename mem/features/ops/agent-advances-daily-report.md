...
- **Data:** `agent_advance_requests` (all rows — flow + rejection reasons + month trend), `agent_advances` (portfolio active/overdue/completed + outstanding), `agent_advance_ledger` (`amount_deducted` = repayments), and the **`agent_ops_qualifying_agent_ids()` RPC for the agent base count** (the canonical "who is an agent" set — ~141, NOT `user_roles` role=agent which is ~22k). KPI label is "Qualifying agents". Profiles read `phone` (not `phone_number`).
