...
## Authenticity e-stamp watermark (2026-07-07)
Every payout receipt now carries the Welile Technologies official e-stamp as a faint authenticity watermark (blue border, serif "WELILE TECHNOLOGIES LIMITED", red current date flanked by ★ stars, "PO Box 167564 Kampala Uganda").
- **`src/components/receipts/WelileStamp.tsx`** — reusable component. `stampDate()` exports the dynamic date in the physical-stamp format (`en-GB`, `DD MON YYYY`, uppercased, e.g. `07 JUL 2026`) — the date is intentionally today's viewing date (user requirement: keep the date dynamic). Fonts (Crimson Text / Oswald / Nunito) are lazy-loaded once via an injected `<link>`. `watermark` prop drops opacity to 0.16.
- **Web receipt** (`PayoutReceipt.tsx`) — stamp overlaid `absolute inset-0 z-20 pointer-events-none` at `scale={0.62}`, centered, readable under the text.
- **PDF** (`payoutReceiptPdf.ts` `drawStampWatermark`) — vector reproduction at opacity 0.14 via jsPDF `GState`, centered at y≈430; uses `*` for stars (core fonts lack ★) and `times` bold for the serif company line. Calls `stampDate()` so the PDF stamp matches the web.
